...
chile_merged <- chile_merged_raw %>%
  ...
  mutate(commune_code = ifelse(nchar(as.character(school_commune_code)) == 4,
                               paste0("0", as.character(school_commune_code)),
                               as.character(school_commune_code)))
```
...
health_service_lookup <- commune_region_lookup %>% group_by(health_service_code, health_service_name) %>% summarise()
...
chile_bayes_aut <- chile_merged %>%
  ...
  mutate(autism = ifelse(special_needs_code == 105, 1, 0),
         ...
         school_fee_group = factor(school_fee_group, levels = c("Free", "Low", "Medium", "No information"))) %>% 
  #left_join(commune_region_lookup, by = c("commune_code")) %>%
  #rename(school_commune_code = commune_code) %>%          
...
chile_bayes_group_temp <- chile_bayes_aut %>%
  group_by(commune_code, school_commune_name, commune_name, health_service_code, health_service_name) %>%
  summarise(count = n())
...
```{r biobio}
# There are issues with the codes for student_commune_code because some of them used to be Biobio but are now Nuble
biobio_nuble_lookup <- chile.adm3 %>%
  filter(#ADM1_PCODE == "CL08" | 
    ADM1_PCODE == "CL16")

ggplot(biobio_nuble_lookup) +
  geom_sf(mapping = aes(geometry = geometry, fill = ADM2_ES)) +
  #geom_sf_text(mapping = aes(geometry = geometry, label = commune_name), size = 3) +
  geom_sf_text(mapping = aes(geometry = geometry, label = commune_code), size = 3) +
  #coord_sf(xlim = c(-74, -70.5), ylim = c(-40, -37.5)) +
  labs(title = "Communes in Nuble") +
  xlab("Longitude") +
  ylab("Latitude")

chile_merged_084xx <- chile_merged %>%
  filter(grepl("084", commune_code) & school_region_name_abr == "BBIO") %>%
  select(school_region_name_abr, school_commune_name, school_commune_code, commune_code)
...
theta_mu_proj <- c(0.005, 0.01, 0.02, 0.03) # 0.5%, 1%, 2%, 3% prevalence
theta_mu_proj_f <- c(0.0025, 0.005, 0.01, 0.02)
theta_mu_proj_m <- c(0.01, 0.02, 0.03, 0.05)
theta_sigma_proj <- c(rep((aut_prev_adj$ci_upper - aut_prev_adj$ci_lower) / (2*1.96), 4)) # Same as chosen prior
...
ggplot(aut_prev_region.geom.f) + 
  ...
  labs(title = "Autism prevalence (females)") +
  xlab("Longitude") +
  ylab("Latitude") +
...
ggplot(aut_prev_region.geom.m) + 
  ...
  geom_sf_text(mapping = aes(geometry = geometry, label = nombre_region), nudge_x = 8, size = 3) +
  scale_fill_viridis(option = "plasma", name = "Prevalence") + #, limits = c(0, 0.025)) +
  labs(title = "Autism prevalence (males)") +
  xlab("Longitude") +
  ylab("Latitude") +
  coord_sf(xlim = c(-83, -55), ylim = c(-60, -15))
...
```{r region proj}
for(j in 1:length(theta_mu_proj)) {
  aut_prev_region_post <- do_jags_rand_model(x = aut_prev_region_adj,
                                             feat = "school_region_name_abr",
                                             model = rand_model,
                                             theta_mu = theta_mu_proj[j],
                                             theta_sigma = theta_sigma_proj[j],
                                             pars = pars, 
                                             convergence_checks = FALSE) %>%
    rename("school_region_name_abr" = "Feat_names")
...
plot_post_density(aut_prev_region_post_f, aut_prev_region_adj_f, feat = "school_region_name_abr", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")
...
plot_post_density(aut_prev_region_post_m, aut_prev_region_adj_m, feat = "school_region_name_abr", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")


```


```{r region by sex proj}
# Females
for(j in 1:length(theta_mu_proj)) {
  aut_prev_region_post_f <- do_jags_rand_model(x = aut_prev_region_adj_f,
                                             feat = "school_region_name_abr",
                                             model = rand_model,
                                             theta_mu = theta_mu_proj_f[j],
                                             theta_sigma = theta_sigma_proj[j],
                                             pars = pars, 
                                             convergence_checks = FALSE) %>%
    rename("school_region_name_abr" = "Feat_names")
...
araucS_upper_prev <- 1688 / aut_prev_health_adj %>%
  filter(health_service_name == "Araucanía Sur") %>%
...
araucS_upper <- 1688
araucS_mean <- (aracuS_upper + araucS_lower) / 2
araucS_sd <- (aracuS_upper - araucS_lower) / (2*1.96)
...
# 
# aut_prev_health_post_new <- do_jags_rand_model(x = aut_prev_health_adj,
#                               feat = "health_service_name",
#                               model = rand_model_new,
#                               theta_mu = (aut_prev_health_adj$new_rate +
#                                             aut_prev_health_adj$adjusted_rate) / 2,
#                               theta_sigma = (aut_prev_health_adj$new_rate -
#                                                aut_prev_health_adj$adjusted_rate) / (2*1.96),
#                               pars = pars, 
...
ggplot() +
  ...
  facet_wrap(~health_service_name) +
  labs(title = "Region specific priors informed by clinical data from Servicio de Salud Araucanía Sur")
...
plot_post_density(aut_prev_ethnic_post_f, aut_prev_ethnic_adj_f, feat = "ethnic_2_group", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")
...
plot_post_density(aut_prev_ethnic_post_m, aut_prev_ethnic_adj_m, feat = "ethnic_2_group", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")


```


```{r ethnicity by sex proj}
# Females
for(j in 1:length(theta_mu_proj)) {
  aut_prev_ethnic_post_f <- do_jags_rand_model(x = aut_prev_ethnic_adj_f,
                                             feat = "ethnic_2_group",
...
## Random effect on economic status - free education, low status, medium status and no information
...
Use grouped fees because the ungrouped classes are too small and there are no females in the 1,000-10,000 group.

```{r economicGroup by sex}
# Females
aut_prev_econG_f <- chile_bayes_aut %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, grouping_vars = c("school_fee_group", "age_june30", "sex", "autism"))

aut_prev_econG_adj_f <- get_adjusted_prev(aut_prev_econG_f, grouping_vars = "school_fee_group")

aut_prev_econG_post_f <- do_jags_rand_model(x = aut_prev_econG_adj_f,
                              feat = "school_fee_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_f,
                              theta_sigma = theta_sigma_prior_f,
...
plot_post_density(aut_prev_econG_post_f, aut_prev_econG_adj_f, feat = "school_fee_group", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")

# Males
aut_prev_econG_m <- chile_bayes_aut %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, grouping_vars = c("school_fee_group", "age_june30", "sex", "autism"))

aut_prev_econG_adj_m <- get_adjusted_prev(aut_prev_econG_m, grouping_vars = "school_fee_group")

aut_prev_econG_post_m <- do_jags_rand_model(x = aut_prev_econG_adj_m,
                              feat = "school_fee_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_m,
                              theta_sigma = theta_sigma_prior_m,
...
plot_post_density(aut_prev_econG_post_m, aut_prev_econG_adj_m, feat = "school_fee_group", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")

```



```{r economic by sex proj}
# Females
for(j in 1:length(theta_mu_proj)) {
  aut_prev_econG_post_f <- do_jags_rand_model(x = aut_prev_econG_adj_f,
                                             feat = "school_fee_group",
  ...
  plot_post_density(aut_prev_econG_post_f, 
                    aut_prev_econG_adj_f, 
                    feat = "school_fee_group", 
                    theta_mu = theta_mu_proj_f[j],
                    theta_sigma = theta_sigma_proj[j],
                    title_text = " (females)")
...
for(j in 1:length(theta_mu_proj)) {
  ...
  plot_post_density(aut_prev_econG_post_m, 
                    aut_prev_econG_adj_m, 
                    feat = "school_fee_group", 
                    theta_mu = theta_mu_proj_m[j],
                    theta_sigma = theta_sigma_proj[j],
                    title_text = " (males)")
...
# ML on who has autism

Use school data? Or some hybrid of school and clinical?

```{r regress autism, include = FALSE}

#fit_log <- glm(autism ~ school_region_name_abr + commune_name + sex + age_june30 + school_rurality_code + school_fee_group + ethnic_2_group, data = chile_bayes_aut)

```

Regression takes waaaay too long to fit.

```{r test train split}
# set.seed(123)
# trainIndex <- createDataPartition(y = chile_bayes_aut, times = 1, p = 0.8, list=F)
# classTrain <- as.factor(cuomoType[trainIndex])
# dataTrain <- cuomoData[trainIndex, ]
# classTest <- as.factor(cuomoType[-trainIndex])
...
set.seed(42)
# Fit decision tree
dtFit <- train(classTrain ~., data = cbind(dataTrain, classTrain),
               method = "rpart",
               parms = list(split = "gini"), # could also try "information"
