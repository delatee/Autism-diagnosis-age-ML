...
rand_model <- "model {
  for(i in 1:nFeat) { # For each category in the feature
    theta[i] ~ dbeta(theta_a, theta_b)
    aut_sample[i] ~ dbin(theta[i], nObs[i])
    aut_pred[i] ~ dbin(theta[i], nObs[i])
  }
...
do_jags_rand_model <- function(x, feat, model, theta_a, theta_b, pars, nBurn = 1000, nIter = 1000, convergence_checks = TRUE) {
  ...
  # theta_a and theta_b single values or vectors of priors for beta distribution
  ...
  for(j in 1:length(theta_a)) {
    # Run JAGS model
    rand_data <- list(theta_a = theta_a[j], 
                      theta_b = theta_b[j],
    ...
    aut_prev_post <- as_tibble(as_draws_matrix(rand_sam), rownames = "Iteration") %>%
      select(c("Iteration", contains("theta["))) %>%
      pivot_longer(cols = contains("theta["),
                   names_to = "Feat",
                   values_to = "predicted_prev") %>%
...
testing <- do_jags_rand_model(x = aut_prev_region_adj,
