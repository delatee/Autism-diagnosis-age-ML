...
reg_mat <- as_draws_matrix(as_draws(rand_region_sam))
aut_prev_reg <- array(data = reg_mat[, paste0("theta[",1:nRegion,"]")], dim = c(nIter, nRegion))
colnames(aut_prev_reg) <- sort(unique(aut_prev_region_adj$school_region_name_abr))
aut_prev_reg_long <- aut_prev_reg %>% pivot_longer()

RegionNames <- sort(unique(aut_prev_region_adj$school_region_name_abr))

aut_prev_reg <- as_tibble(as_draws_matrix(rand_region_sam), rownames = "Iteration") %>%
...
aut_prev_region_adj <- aut_prev_region_adj %>%
  left_join(aut_prev_reg %>% group_by(school_region_name_abr) %>% summarise(post_lower = quantile(predicted_prev, 0.025),
                                                                            post_upper = quantile(predicted_prev, 0.975)),
            by = "school_region_name_abr")

#aut_prev_reg_ci <- aut_prev_reg %>% 
#  group_by(Region_name) %>% 
#  summarise(post_lower = quantile(predicted_prev, 0.025), post_upper = quantile(predicted_prev, 0.975)) %>%
  

ggplot(aut_prev_reg) +
  geom_density(aes(x = predicted_prev)) +
  geom_vline(data = aut_prev_region_adj, aes(xintercept = post_lower), color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_region_adj, aes(xintercept = post_upper), color = "blue", linetype = "dotted") +
