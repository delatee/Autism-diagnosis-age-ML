...
regions_lookup <- communes_lookup %>% group_by(codigo_region, nombre_region) %>% summarise()

region_geoms <- generar_circunscripciones(mapa = chilemapas::mapa_comunas) %>%
  left_join(regions_lookup, by = "codigo_region") %>%
...
ggplot(region_geoms) + 
  ...
  scale_fill_viridis(option = "viridis", direction = -1, name = "Population") +
...
ggplot(data = aut_prev_region.agecat) +
  geom_col(aes(x = school_region_name_abr, y = sample_prevalence, group = age_cat_name, fill = age_cat_name), position = position_dodge()) +
  geom_errorbar(aes(x = school_region_name_abr, ymin = ci_lower, ymax = ci_upper, group = age_cat_name), width = 0.2, position = position_dodge(width = 0.9)) +
...
ggplot(data = aut_prev_region.age) +
  geom_col(aes(x = school_region_name_abr, y = sample_prevalence, group = age_june30, fill = as.factor(age_june30)), position = "dodge") +
  geom_errorbar(aes(x = school_region_name_abr, ymin = ci_lower, ymax = ci_upper, group = age_june30), width = 0.2, position = position_dodge(width = 0.9)) +
  scale_fill_viridis_d() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Autism prevalence",
       x = "School region",
