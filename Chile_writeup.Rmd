...
region_service_commune_lookup <- chile.adm3 %>%
  ...
  filter(!is.na(ADM3_PCODE)) %>% 
    # Get rid of the communes that didn't match properly with health service lookup and were corrected above
...
chile_regionpop_raw <- read_excel("04_Data/ine_estimaciones-y-proyecciones-2002-2035_base-2017_region_area_tabulados.xlsx",
...
# http://www.ine.gob.cl/estadisticas/sociales/censos-de-poblacion-y-vivienda/censo-de-poblacion-y-vivienda
...
chile_bayes <- chile_merged %>%
  ...
  mutate(autism = ifelse(special_needs_code == 105, 1, 0),
         ...
         ethnic_3_group = factor(ifelse(ethnic_3_group == "Mapuche", "Mapuche",
                                 ifelse(ethnic_3_group == "Aymara", "Aymara",
                                 ifelse(ethnic_3_group == "No Native Group", "No Indigenous group",
                                 "Other Indigenous group"))),
                          levels = c("Mapuche", "Aymara", "No Indigenous group", "Other Indigenous group")),
...
# Population
ggplot(demog_geom) + 
  geom_sf(mapping = aes(geometry = geometry, fill = youth_pop_2021), 
          linewidth = 0.001, colour = "#555555") +
  geom_sf_text(mapping = aes(geometry = geometry, label = region_name), nudge_x = 9, size = 3.5) +
  scale_fill_viridis(option = "viridis", direction = 1, name = "Population aged 0-14") + 
...
```{r mapIncome, include = TRUE, fig.cap = "Net income from main job in Chile in 2021 by region, from the INE's Supplementary Income Survey (10).", fig.height=8}
# Income
ggplot(demog_geom) + 
...
```{r mapRural, include = TRUE, fig.cap = "Percentage of population living in rural areas in Chile in 2017 from 2017 census (13).", fig.height=8}
# Rurality
ggplot(demog_geom) + 
...
See Figure \@ref(fig:sampleSize) for an illustration of the data sources and how they were combined through the analysis detailed below.
...
```{r mapArauc, fig.cap = "Communes in the La Araucanía region, with Servicio de Salud Araucanía Sur (SSAS) communes in green and Servicio de Salud Araucanía Norte (SSAN) communes in blue (60).", fig.height=8}
# ARAUC communes
...
For mapping, shapefiles of administrative areas were obtained from The Humanitarian Data Exchange of the United Nations Office for the Coordination of Humanitarian Affairs (60). Additional region and commune naming information was taken from the R package ‘chilemapas’. 


## Aim 1: Use school data and frequentist method to find a lower bound on autism and ADHD prevalence, and assess socio-demographic variation
...
The crude prevalence of autism and of ADHD were calculated as the count of cases divided by the relevant population size. Grouping features of sex, health service of residential commune, monthly school fee as a proxy for SES, ethnicity and school’s rurality were used, and calculations were made by sex and age band as calculating by single year ages resulted in too many counts of less than 20. 
...
## Aim 3b: Accurately estimate autism prevalence and unmet need, project estimates across health services using Bayesian prevalence prediction
...
Bayesian prevalence analysis of autism was conducted for the school data by health service. Bayesian methods were appropriate here because they allowed calculation of prevalence with incomplete data. Here two different types of data, school records and patient appointments, were combined and both were known to be incomplete. Bayesian analysis is robust to this messiness and allowed plausible prevalence predictions to be made. It also provided information about the likelihood of these predictions given the observed school data.
...
Second, a conjugate beta prior specific to each health service using the health service specific age- and sex-adjusted autism prevalence in the school data and their standard deviations as the prior means and their standard deviations respectively. This prior was suitable because it extended the previous prior to each of the random effect categories and it reflected the students know to be receiving SEED for autism. On its own this prior was expected to give uninformative posteriors because it was effectively duplicating the information in the sample data. However it was used here as a health service specific lower bound on the plausible prevalence of autism that can then be compared to the projected prevalence values. 
...
```{r sampleSize, fig.cap="The datasets used in this investigation, their size (N) and the number of people with autism and ADHD and their crude prevalence where applicable. Light blue shows the school data which was used to calculate lower bounds on autism and ADHD prevalence, and subsets of the school data for students resident in Servicio de Salud Araucanía Sur (SSAS) and those that were not. In the school data, autism and ADHD indicate students that accessed SEED for autism and for ADHD respectively. Green shows the clinical data and its subsets of validated and unvalidated clinical data and autism here indicates clinical diagnoses. Validation involved confirming autism diagnosis against central clinical records, adding 2nd and 3rd diagnoses, and adding demographic data. Red shows the validated clinical data for patients resident in SSAS that was used for multiple correspondence analysis. Yellow shows the subsets of school and clinical data used for data linkage; the patient data was the combination of validated and unvalidated clinical datasets and some patients were represented in both these datasets, shown by the dotted overlap boxes. Grey shows the matches resulting from three linkage approaches. Dark blue shows the result of data linkage and was used to calculated updated estimates of autism prevalence.", echo=FALSE, out.width = '100%'}
knitr::include_graphics("sample_sizes.jpg", error = FALSE)
```

Figure \@ref(fig:sampleSize) shows the school and clinical data and how they contribute to the three aims of this investigation. The school data is used to address aim 1, finding a lower bound on autism and ADHD prevalence and understanding their variation, see sections [5.1][School data] and [5.2][Frequentist prevalence estimation]. The validated clinical data for patients resident in SSAS is used to address aim 2, identifying common characteristics of children with autism, see sections [5.3][Clinical data] and [5.4][Machine learning with clinical data]. Linkage of school and clinical data is used to address aim 3, obtaining and projecting accurate autism prevalence estimates, see sections [5.5][Linkage of school and patient records], [5.6][Updated autism prevalence estimates and delta] and [5.7][Bayesian prevalence projection].
...
kbl(school.table,
    ...
    caption = "Count and percentage of features' values in the school dataset. Metro. is short for Metropolitano and these health services are all in Santiago.") %>%
  kable_styling(latex_options = c("repeat_header"))
```

The school dataset contained records for `r format(nrow(chile_bayes), big.mark = ",", trim = TRUE)` Chilean students aged 6-18 in 2021, see Table \@ref(tab:schoolTable). Of these, `r format(chile_bayes %>% filter(sex_desc == "Female") %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(sex_desc == "Female") %>% nrow() / nrow(chile_bayes) * 100, 2)`%) were female and the rest were male. Students were evenly distributed across age bands and fairly evenly distributed across communes with generally more students in the metropolitan Santiago communes. Most students received free schooling and after this value, the proportion that paid for schooling increased with monthly school fees. `r format(chile_bayes %>% filter(ethnic_2_group == "Mapuche") %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(ethnic_2_group == "Mapuche") %>% nrow() / nrow(chile_bayes) * 100, 2)`%) students were Mapuche and `r format(chile_bayes %>% filter(ethnic_2_group == "No Indigenous group") %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(ethnic_2_group == "No Indigenous group") %>% nrow() / nrow(chile_bayes) * 100, 2)`%) did not belong to an Indigenous group. `r format(chile_bayes %>% filter(school_rurality == "Rural") %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(school_rurality == "Rural") %>% nrow() / nrow(chile_bayes) * 100, 2)`%)a ttended a school in a rural area. A special needs code was recorded for `r format(chile_bayes %>% filter(special_needs_status == 1) %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(special_needs_status == 1) %>% nrow() / nrow(chile_bayes) * 100, 2)`%) students, indicating they accessed SEED funding in 2021. Of these students, `r format(chile_bayes %>% filter(autism == 1) %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(autism == 1) %>% nrow() / chile_bayes %>% filter(special_needs_status == 1) %>% nrow() * 100, 2)`%) received SEED for autism and `r format(chile_bayes %>% filter(adhd == 1) %>% nrow(), big.mark = ",", trim = TRUE)` (`r round(chile_bayes %>% filter(adhd == 1) %>% nrow() / chile_bayes %>% filter(special_needs_status == 1) %>% nrow() * 100, 2)`%) received SEED for ADHD.
...
The national crude prevalence of autism in the school data was `r round(aut_prev_adj$crude_rate * 100, 2)`% (`r round(aut_prev_adj$crude_ci_lower * 100, 2)`-`r round(aut_prev_adj$crude_ci_upper * 100, 2)`%). After using frequentist methods to adjust for the age and sex distribution of the 6-18 year-old population of Chile in 2021, the national adjusted prevalence of autism was `r round(aut_prev_adj$adjusted_rate * 100, 2)`% (`r round(aut_prev_adj$adjusted_ci_lower * 100, 2)`-`r round(aut_prev_adj$adjusted_ci_upper * 100, 2)`%). The national crude prevalence of ADHD was `r sprintf("%.2f", round(adhd_prev_adj$crude_rate * 100, 2))`% (`r sprintf("%.2f", round(adhd_prev_adj$crude_ci_lower * 100, 2))`-`r round(adhd_prev_adj$crude_ci_upper * 100, 2)`%) and the national age- and sex-adjusted prevalence was `r sprintf("%.2f", round(adhd_prev_adj$adjusted_rate * 100, 2))`% (`r sprintf("%.2f", round(adhd_prev_adj$adjusted_ci_lower * 100, 2))`-`r round(adhd_prev_adj$adjusted_ci_upper * 100, 2)`%). The crude prevalence of autism and ADHD varied with age, as shown in Figures \@ref(fig:prevAgecatAutPlot) and \@ref(fig:prevAgecatAdhdPlot). Autism prevalence was highest in 6-8 year-olds and decreased with age while ADHD prevalence peaked in 9-11 year-olds then decreased. Both conditions showed a small increase in prevalence for age 18. See also Supplementary Tables \@ref(tab:prevAgecatAutTable) and \@ref(tab:prevAgecatAdhdTable).

```{r prevAgecatAutPlot, fig.cap = "Crude prevalence of autism in school data by age band. Bars show 95% normal confidence intervals.", fig.height = 3}
# Autism
...
```{r prevAgecatAdhdPlot, fig.cap = "Crude prevalence of ADHD in school data by age band. Bars show 95% normal confidence intervals.", fig.height = 3}
# ADHD
...
Disggregated by sex, autism prevalence in the school data was `r round(aut_prev_sex_adj$crude_rate[2] * 100, 2)`% (`r round(aut_prev_sex_adj$crude_ci_lower[2] * 100, 2)`-`r round(aut_prev_sex_adj$crude_ci_upper[2] * 100, 2)`%) for females and `r sprintf("%.2f", round(aut_prev_sex_adj$crude_rate[1] * 100, 2))`% (`r round(aut_prev_sex_adj$crude_ci_lower[1] * 100, 2)`-`r round(aut_prev_sex_adj$crude_ci_upper[1] * 100, 2)`%) for males, with male to female ratio of `r round(aut_prev_sex_adj$crude_rate[1] / aut_prev_sex_adj$crude_rate[2], 2)`:1, see Figure \@ref(fig:prevAgecatSexAutPlot) and Supplementary Table \@ref(tab:prevAgecatSexAutTable). Among females, the age- and sex-adjusted autism prevalence was `r round(aut_prev_sex_adj$adjusted_rate[2] * 100, 2)`% (`r round(aut_prev_sex_adj$adjusted_ci_lower[2] * 100, 2)`-`r round(aut_prev_sex_adj$adjusted_ci_upper[2] * 100, 2)`%) and among males it was `r round(aut_prev_sex_adj$adjusted_rate[1] * 100, 2)`% (`r round(aut_prev_sex_adj$adjusted_ci_lower[1] * 100, 2)`-`r sprintf("%.2f", round(aut_prev_sex_adj$adjusted_ci_upper[1] * 100, 2))`%), thus giving an adjusted male to female ratio of `r sprintf("%.2f", round(aut_prev_sex_adj$adjusted_rate[1] / aut_prev_sex_adj$adjusted_rate[2], 2))`:1. ADHD prevalence was `r round(adhd_prev_sex_adj$crude_rate[2] * 100, 2)`% (`r sprintf("%.2f", round(adhd_prev_sex_adj$crude_ci_lower[2] * 100, 2))`-`r round(adhd_prev_sex_adj$crude_ci_upper[2] * 100, 2)`%) for females and `r sprintf("%.2f", round(adhd_prev_sex_adj$crude_rate[1] * 100, 2))`% (`r round(adhd_prev_sex_adj$crude_ci_lower[1] * 100, 2)`-`r round(adhd_prev_sex_adj$crude_ci_upper[1] * 100, 2)`%) for males, with male to female ratio of `r round(adhd_prev_sex_adj$crude_rate[1] / adhd_prev_sex_adj$crude_rate[2], 2)`:1, see Figure \@ref(fig:prevAgecatSexAdhdPlot) and Supplementary Table \@ref(tab:prevAgecatSexAdhdTable). After adjustment, the prevalence of ADHD was `r sprintf("%.2f", round(adhd_prev_sex_adj$adjusted_rate[2] * 100, 2))`% (`r sprintf("%.2f", round(adhd_prev_sex_adj$adjusted_ci_lower[2] * 100, 2))`-`r round(adhd_prev_sex_adj$adjusted_ci_upper[2] * 100, 2)`%) for females and `r sprintf("%.2f", round(adhd_prev_sex_adj$adjusted_rate[1] * 100, 2))`% (`r sprintf("%.2f", round(adhd_prev_sex_adj$adjusted_ci_lower[1] * 100, 2))`-`r round(adhd_prev_sex_adj$adjusted_ci_upper[1] * 100, 2)`%) for males with adjusted male to female ratio of `r round(adhd_prev_sex_adj$adjusted_rate[1] / adhd_prev_sex_adj$adjusted_rate[2], 2)`:1. 
...
```{r prevAgecatSexAutPlot, fig.cap = "Crude prevalence of autism in school data by age band and sex. Bars show 95% normal confidence intervals.", fig.height = 3}
# Autism
...
```{r prevAgecatSexAdhdPlot, fig.cap = "Crude prevalence of ADHD in school data by age band and sex. Bars show 95% normal confidence intervals.", fig.height = 3}
# ADHD
...
Considering school data by health service, shown in Table \@ref(tab:adjPrevHealthAutTable), the adjusted prevalence of autism was significantly higher in Ñuble at `r round(aut_prev_health_adj$adjusted_rate[29] * 100, 2)`% (`r round(aut_prev_health_adj$adjusted_ci_lower[29] * 100, 2)` - `r round(aut_prev_health_adj$adjusted_ci_upper[29] * 100, 2)`%) than other health services with non-overlapping confidence intervals. Adjusted prevalence was lowest in Metropolitano Norte at `r sprintf("%.2f", round(aut_prev_health_adj$adjusted_rate[17] * 100, 2))`% (`r round(aut_prev_health_adj$adjusted_ci_lower[17] * 100, 2)` - `r sprintf("%.2f", round(aut_prev_health_adj$adjusted_ci_upper[17] * 100, 2))`%) and Araucanía Norte at `r sprintf("%.2f", round(aut_prev_health_adj$adjusted_rate[4] * 100, 2))`% (`r round(aut_prev_health_adj$adjusted_ci_lower[4] * 100, 2)`- `r round(aut_prev_health_adj$adjusted_ci_upper[4] * 100, 2)`%). Autism prevalence peaked in the 6-8 age band across all health services except Chiloé and Magallanes where it peaked in the 9-11 band, see Supplementary Table  \@ref(tab:prevAgecatHealthAutTable) and Supplementary Figure \@ref(fig:prevAgecatHealthAutPlot).

ADHD prevalence also varied across health services, as shown in Table \@ref(tab:adjPrevHealthAdhdTable). Prevalence was significantly lower in Atacama than other regions at `r round(adhd_prev_health_adj$adjusted_rate[8] * 100, 2)`% (`r round(adhd_prev_health_adj$adjusted_ci_lower[8] * 100, 2)` - `r round(adhd_prev_health_adj$adjusted_ci_upper[8] * 100, 2)`%). Magallanes, Talcahuano, Concepción, Chiloé, Biobío, Aisén, Ñuble, Aconcagua and Coquimbo all had adjusted prevalence of 2.00% or higher and there was a significant gap between these health services and those with lower prevalence. There was also a significant gap separating Magallanes, Talcahuano, Concepción and Chiloé as the highest prevalence health services. See also Supplementary Table  \@ref(tab:prevAgecatHealthAdhdTable) and Supplementary Figure \@ref(fig:prevAgecatHealthAdhdPlot).
...
For school fees, which were used here as a proxy for SES, autism prevalence was highest among students that received free or low fee education, though the sample size for students that paid \$1,000-\$10,000 monthly was very small, see Table \@ref(tab:adjPrevEconAAutTable). ADHD prevalence was variable across school fee levels, with the \$1,000-\$10,000 band having low prevalence and very few cases, see Table \@ref(tab:adjPrevEconAAdhdTable). For both autism and ADHD, prevalence was very low among students paying more than \$100,000 monthly, suggesting students from wealthier families may not have accessed SEED or may not have been eligible for it due to attending non-subsidised private schools. See also Supplementary Tables \@ref(tab:prevAgecatEconAAutTable) and \@ref(tab:prevAgecatEconAAdhdTable) and Supplementary Figures \@ref(fig:prevAgecatEconAAutPlot), \@ref(fig:prevAgecatEconAAdhdPlot).
...
For both autism and ADHD, adjusted prevalence was significantly lower for Mapuche students than non-Indigenous students in the Aysén, Biobío, La Araucanía, Los Lagos, Los Rios, Magallanes and Región Metropolitana de Santiago regions, as shown in Tables \@ref(tab:adjPrevEthnicAutTable) and \@ref(tab:adjPrevEthnicAdhdTable). Autism prevalence for students belonging to other Indigenous groups fall in between and was not significantly different, and ADHD prevalence for other Indigenous groups was lower but not significantly. See also Supplementary Tables \@ref(tab:prevAgecatEthnicAutTable) and  \@ref(tab:prevAgecatEthnicAdhdTable) and Supplementary Figures \@ref(fig:prevAgecatEthnicAutPlot) and \@ref(fig:prevAgecatEthnicAdhdPlot). 
...
Autism and ADHD were both significantly more prevalent for students at rural schools than urban schools at `r round(aut_prev_rural_adj$adjusted_rate[1] * 100, 2)`% (`r round(aut_prev_rural_adj$adjusted_ci_lower[1] * 100, 2)` - `r round(aut_prev_rural_adj$adjusted_ci_upper[1] * 100, 2)`%) and `r round(aut_prev_rural_adj$adjusted_rate[2] * 100, 2)`% (`r round(aut_prev_rural_adj$adjusted_ci_lower[2] * 100, 2)` - `r round(aut_prev_rural_adj$adjusted_ci_upper[2] * 100, 2)`%) respectively for autism and `r round(adhd_prev_rural_adj$adjusted_rate[1] * 100, 2)`% (`r round(adhd_prev_rural_adj$adjusted_ci_lower[1] * 100, 2)` - `r round(adhd_prev_rural_adj$adjusted_ci_upper[1] * 100, 2)`%) and `r round(adhd_prev_rural_adj$adjusted_rate[2] * 100, 2)`% (`r round(adhd_prev_rural_adj$adjusted_ci_lower[2] * 100, 2)` - `r sprintf("%.2f", round(adhd_prev_rural_adj$adjusted_ci_upper[2] * 100, 2))`%) respectively for ADHD, as shown in Tables \@ref(tab:adjPrevRuralAutTable) and \@ref(tab:adjPrevRuralAdhdTable). See also Supplementary Tables \@ref(tab:prevAgecatRuralAutTable) and \@ref(tab:prevAgecatRuralAdhdTable) and Supplementary Figures \@ref(fig:prevAgecatRuralAutPlot) and \@ref(fig:prevAgecatRuralAdhdPlot).
...
```{r mcaPatientsSmallOutput, include = FALSE}
patients_mca_small_6_18 <- patients_mca_small %>%
  mutate(age = as.numeric(age)) %>%
  filter(age >= 6 & age <= 18)

write.csv(patients_mca_small_6_18, "04_Data/Outputs/patients_ssas_6_18.csv")
```

The validated clinical dataset has data on `r format(nrow(clinical_small), big.mark = ",", trim = TRUE)` appointments for `r length(unique(clinical_small$id))` unique patients with autism aged 1-18 in 2021, of which `r patients_mca_small %>% nrow()` patients have lived in a commune in the SSAS catchment area during the period the data covers. Among these, most were male, live in urban areas and have Chilean but not Mapuche ethnicity. The mean age is `r round(mean(as.numeric(patients_mca_small$age)), 1)` years. Many patients live in Villarica commune as the validated clinical data is from Villarica Hospital. FONASA-A, indicative of low SES, is the most common level of health insurance, and few patients have another recorded disability or have experienced foster care. 
...
Multiple correspondence analysis was first conducted with all features thought to be associated with autism diagnosis with no imputation. Figure \@ref(fig:mcaNoimpFeatures) shows approximately 14.6% of the variance in these data were captured by the first two dimensions of MCA. Disability and foster care status were well separated by the first dimension but Figures \@ref(fig:mcaNoimpDisab) and \@ref(fig:mcaNoimpFoster) show that this separation was primarily driven by whether information was available for these features. Ethnicity, age band and commune of residence were well separately by the second dimension of MCA, as shown in Figure \@ref(fig:mcaNoimpFeatures), and were somewhat separated by the first dimension. Figures \@ref(fig:mcaNoimpGraph) and \@ref(fig:mcaNoimpCategories) further show the importance of categories within the foster care, disabilities, ethnicity, age band and commune features for explaining the variance in this data. In particular, categories that explained more of the variance include not having disability or foster care status, or not having information on these, having age of 0-2 or 3-5, being foreign or Chilean, living in Toltén and having private health insurance. Examining the clustering of individual patients by the first two dimensions of MCA, Figure \@ref(fig:mcaNoimpAge) demonstrates that patients in age bands 0-2 and 3-5 clustered well and those in older age bands did not. There was some clustering by ethnicity in Figure \@ref(fig:mcaNoimpEthnic) but it was obscured by the separation of points into the two larger clustered defined by having or not having information on disability and foster care status. Figure \@ref(fig:mcaNoimpCommune) shows clear separation of Toltén and Nueva Imperial communes, however it is important to note here that these and several other communes were represented by only one patient, see Table \@ref(tab:macPatientsSmallTable). There was decent clustering of patients in Temuco and Pitrufquén communes. Figure \@ref(fig:mcaNoimpSES) shows possible separation for patients with private health insurance and Figures \@ref(fig:mcaNoimpSex) and \@ref(fig:mcaNoimpRural) show little clustering by sex or rurality of residence.
...
```{r mcaNoimpFeatures, fig.cap="Categorical features by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation.", fig.height=3}
# Plot features
...
```{r mcaNoimpGraph, fig.cap="Contribution of the top 15 categories to the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation. The red line shows the expected average if contributions were uniform.", fig.height=3}
...
```{r mcaNoimpCategories, fig.cap="Available categories by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation. Brigther, more yellow colours indicate larger contribution to the first two dimensions.", fig.height=10, fig.width=8.5}
# Plot categories
fviz_mca_var(res_mca_patients_noimp,
             col.var = "contrib",
             labelsize = 4, 
...
```{r mcaNoimpAge, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by age band.", fig.height=3}
mca_noimp_age_plot
```

```{r mcaNoimpEthnic, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by ethnicity.", fig.height=3}
mca_noimp_ethnic_plot
```

```{r mcaNoimpCommune, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by commune of residence.", fig.height=3}
mca_noimp_commune_plot
```

```{r mcaNoimpSES, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by health insurance level.", fig.height=3}
mca_noimp_ses_plot
```

```{r mcaNoimpSex, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by sex.", fig.height=3}
mca_noimp_sex_plot
```

```{r mcaNoimpRural, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by rurality of residence.", fig.height=3}
mca_noimp_rural_plot
```

```{r mcaNoimpDisab, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by disability status.", fig.height=3}
mca_noimp_disab_plot
```

```{r mcaNoimpFoster, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features without imputation, coloured by foster care status.", fig.height=3}
mca_noimp_foster_plot
```

Exchanging disability and foster care status for their imputed versions led to the MCA capturing approximately 14.1% of the variance in that data with its first two dimensions, as shown in Figure \@ref(fig:mcaImpFeatures). Disability and foster care status were no longer well separated by the first dimension and Figures \@ref(fig:mcaImpDisab) and \@ref(fig:mcaImpFoster) show that the patients who had experienced foster care did cluster well but the patients with a disability did not. In Figure \@ref(fig:mcaImpFeatures), age band and ethnicity were now well separated by both dimensions and commune mostly by the second. With the reduced importance of disability and foster care, age bands 0-2 and 3-5, foreign, no information and Chilean ethnicity and Toltén and Nueva Imperial communes contributed most to the first two dimensions, see Figures \@ref(fig:mcaImpGraph) and \@ref(fig:mcaImpCategories). Again patients with age bands 0-2 and 3-5 clustered well and older age bands did not, as shown in Figure \@ref(fig:mcaImpAge). Figure \@ref(fig:mcaImpEthnic) shows much clearer clustering of ethnicity than before. For communes with more than one patient resident, clustering was less clear than before with only Pitrufquén well separated by these dimensions, see Figure \@ref(fig:mcaImpCommune). Clustering by SES, sex and rurality was weak, see Figures \@ref(fig:mcaImpSES), \@ref(fig:mcaImpSex) and \@ref(fig:mcaImpRural).
...
```{r mcaImpFeatures, fig.cap="Categorical features by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation.", fig.height=3}
# Plot features
...
```{r mcaImpGraph, fig.cap="Contribution of the top 15 categories to the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation. The red line shows the expected average if contributions were uniform.", fig.height=3}
# Total contribution to dimension 1 and 2
...
```{r mcaImpCategories, fig.cap="Available categories by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation. Brigther, more yellow colours indicate larger contribution to the first two dimensions.", fig.height=10, fig.width=8.5}
# Plot categories
fviz_mca_var(res_mca_patients_imp,
             col.var = "contrib",
             labelsize = 4, 
...
```{r mcaImpAge, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by age band.", fig.height=3}
mca_imp_age_plot
```

```{r mcaImpEthnic, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by ethnicity.", fig.height=3}
mca_imp_ethnic_plot
```

```{r mcaImpCommune, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by commune of residence.", fig.height=3}
mca_imp_commune_plot
```

```{r mcaImpSES, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by health insurance level.", fig.height=3}
mca_imp_ses_plot
```

```{r mcaImpSex, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by sex.", fig.height=3}
mca_imp_sex_plot
```

```{r mcaImpRural, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by rurality of residence.", fig.height=3}
mca_imp_rural_plot
```

```{r mcaImpDisab, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by disability status.", fig.height=3}
mca_imp_disab_plot
```

```{r mcaImpFoster, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using all features with imputation, coloured by foster care status.", fig.height=3}
mca_imp_foster_plot
```

MCA on the validated clinical data using only age band, ethnicity and commune resulted in the first two dimensions capturing 17.8% of the variance in that data, see Figure \@ref(fig:mcaAutFeatures), and these features were fairly well represented by the first two dimensions. Again age bands 0-2 and 3-5, foreign, no information and Chilean ethnicity and Toltén and Nueva Imperial communes contributed most to the first two dimensions, see Figures \@ref(fig:mcaAutGraph) and \@ref(fig:mcaAutCategories). By patient, age bands 0-2 and 3-5 still clustered well (Figure \@ref(fig:mcaAutAge)), ethnicity clusters were very distinct (Figure \@ref(fig:mcaAutEthnic)), and communes showed more structure than previously (Figure \@ref(fig:mcaAutCommune)).
...
```{r mcaAutFeatures, fig.cap="Categorical features by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using patients' age band, commune of residence and ethnicity.", fig.height=3}
# Plot features
...
```{r mcaAutGraph, fig.cap="Contribution of the top 15 categories to the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using patients' age band, commune and ethnicity. The red line shows the expected average if contributions were uniform.", fig.height=3}
...
```{r mcaAutCategories, fig.cap="Available categories by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using patients' age band, commune and ethnicity. Brigther, more yellow colours indicate larger contribution to the first two dimensions.", fig.height=10, fig.width=8.5}
# Plot categories
fviz_mca_var(res_mca_patients_aut,
             col.var = "contrib",
             labelsize = 4, 
...
```{r mcaAutAge, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using patients' age band, commune and ethnicity, coloured by age band.", fig.height=3}
mca_aut_age_plot
```

```{r mcaAutEthnic, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using age band, commune and ethnicity, coloured by ethnicity.", fig.height=3}
mca_aut_ethnic_plot
```

```{r mcaAutCommune, fig.cap="Patients by the first two dimensions of multiple correspondence analysis on autism patients in the validated clinical data using age band, commune and ethnicity, coloured by commune of residence.", fig.height=3}
...
school_ssas_n_male <- school_ssas %>% filter(sex_desc == "Male") %>% nrow()
school_ssas_mean_age <- mean(school_ssas$age_june30)
school_ssas_sd_age <- sd(school_ssas$age_june30)
...
Table \@ref(tab:schoolChisq) shows that students living in SSAS were significantly different to students living in the other health services with regard to age band, SES, ethnicity, rurality and whether they accessed SEED at all, but did not differ by sex. 
...
kbl(school_sass_aut.table,
    ...
    linesep = c("", "\\addlinespace", # Sex
                ...
                "", "", "", "", "", "\\addlinespace", # Commune
                "", "", "", "\\addlinespace"), # SES
    caption = "Count and percentage of features' values in the SSAS school dataset which comprises students with autism resident in the SSAS catchment. Proxy SES is school fee group.") %>%
  kable_styling(latex_options = c("repeat_header"))
```

In the school records, `r format(school_ssas %>% nrow(), big.mark = ",", trim = TRUE)` students lived in the SSAS health service catchment, of which `r nrow(school)-1` (`r round((nrow(school)-1) / nrow(school_ssas) * 100, 2)`%) had autism. Among the students with autism, shown in Table  \@ref(tab:ssasSchoolTable), there were many more males than females, Temuco was the most common commune of residence and most students had a proxy SES of 1. 
```{r patients}
...
Using perfect match on sex, date of birth, commune of residence and the proxies for SES, `r nrow(merged_ses)` perfect matches were manually found between the SSAS school and patient records. Of these, `r length(unique(merged_ses$id.x))` unique SSAS school records were perfectly matched to SSAS patient records, and all perfect matches were for unique patients. When mismatch on SES proxy was allowed, `r nrow(merged)` perfect matches were manually found between the SSAS school and patient records. Of these, `r length(unique(merged$id.x))` unique school records were perfectly matched to SSAS patient records and `r length(unique(merged$id.y))` SSAS patients were perfectly matched to SSAS school records.
...
Analysis of differences between matched and unmatched records in the SSAS school data and in the patient data by sex, commune and proxy for SES are provided in the Supplementary Figures. Kolmogorov-Smirnov permutation tests found no significant difference in frequency of sexes between matched (`r round(school_match.sex$freq[2] * 100, 2)`% female) and unmatched SSAS school records (`r round(school_match.sex$freq[1] * 100, 2)`% female), see Supplementary Figure \@ref(fig:kolmogorovSchoolSex). They found a strongly significant difference in the frequency of sexes between matched (`r round(patients_match.sex$freq[2] * 100, 2)`% female) and unmatched patient records (`r round(patients_match.sex$freq[1] * 100, 2)`% female), see Supplementary Figure \@ref(fig:kolmogorovPatientSex). This difference was likely due to the male to female ratios differing across the datasets: the SSAS school data of students with autsim was `r round(school %>% filter(sex_desc == "Female") %>% nrow() / nrow(school) * 100, 2)`% female, the patient data was `r round(patients %>% filter(sex_desc == "Female") %>% nrow() / nrow(patients) * 100, 2)`% female and the matches were `r sprintf("%.2f", round(school_matched_small %>% filter(sex_desc.school == "Female") %>% nrow() / school_matched_small %>% nrow() * 100, 2))`% females. Permutation testing found that for the SSAS school data, matched (`r round(school_match.student_commune_name$freq[31] * 100, 2)`% resident in Temuco) and unmatched records (`r round(school_match.student_commune_name$freq[30] * 100, 2)`% resident in Temuco) differed significantly by commune, see Figure \@ref(fig:kolmogorovSchoolCommune), and that there was no significant difference in the patient data by commune between matched (`r round(patients_match.student_commune_name$freq[33] * 100, 2)`% resident in Temuco) and unmatched records (`r round(patients_match.student_commune_name$freq[32] * 100, 2)`% resident in Temuco), see Figure \@ref(fig:kolmogorovSchoolCommune). This appeared to be driven by the matchability of students and patients living in Temuco, the most prevalent commune. For the SSAS school data by proxy SES there was a strongly significant difference between matched (`r round(school_match.ses_status$freq[2] * 100, 2)`% status of 1, `r round(school_match.ses_status$freq[4] * 100, 2)`% status of 2, 0.00% status of 3 and `r round(school_match.ses_status$freq[7] * 100, 2)`% with unknown status) and unmatched (`r round(school_match.ses_status$freq[1] * 100, 2)`% status of 1, `r round(school_match.ses_status$freq[3] * 100, 2)`% status of 2, `r round(school_match.ses_status$freq[5] * 100, 2)`% status of 3 and `r sprintf("%.2f", round(school_match.ses_status$freq[6] * 100, 2))`% with unknown status) records, see Figure \@ref(fig:kolmogorovSchoolSes). For the patient data there was a somewhat significant difference between matched (`r round(patients_match.ses_status$freq[2] * 100, 2)`% status of 1, `r round(patients_match.ses_status$freq[4] * 100, 2)`% status of 2, and `r round(patients_match.ses_status$freq[6] * 100, 2)`% status of 3) and unmatched (`r round(patients_match.ses_status$freq[1] * 100, 2)`% status of 1, `r round(patients_match.ses_status$freq[3] * 100, 2)`% status of 2 and `r round(patients_match.ses_status$freq[5] * 100, 2)`% status of 3) records, see Figure \@ref(fig:kolmogorovPatientSes). Again this likely reflected different frequencies of the SES values across datasets. Kolmogorov-Smirnov permutation testing was not conducted for date of birth as this feature contained too many categories for results to be meaningful.
...
n_aut_araucS <- nrow(school_matched) + 
  length(unique(patients_matched$patient_id)) - nrow(matches) 
# OR 
n_aut_araucS <- school_matched_small %>% filter(matched == 0) %>% nrow() +
  dim(patients_matched_small_dedup %>% filter(matched == 0) %>% select(patient_id)
      %>% unique())[1] + nrow(matches)
```
...
aut_newprev_araucS_adj_15 <- get_adjusted_prev(rename(aut_newprev_araucS_15,
                                                      "n_disease" = "yes_autism_linked"),
                                              grouping_vars = c()) %>%
  mutate(#sex_desc = "Male",
         age_cat_name = "15-18") %>%
...
After linking the school and patient data for SSAS, `r format(patients_matched_small_dedup %>% filter(matched == 0) %>% nrow(), big.mark = ",", trim = TRUE)` patients with autism could not be matched to students. This represents the unmet need of SSAS students with autism that did not access school-based support for it. Combining these additional cases with the `r school_ssas %>% filter(autism == 1) %>% nrow()` students that did access SEED for autism gave `r format(aut_newprev_araucS_adj$crude_count, big.mark = ",", trim = TRUE)` people with autism in SSAS. Thus the crude updated prevalence of autism in SSAS was `r round(aut_newprev_araucS_adj$crude_rate * 100, 2)`% (`r round(aut_newprev_araucS_adj$crude_ci_lower * 100, 2)`-`r round(aut_newprev_araucS_adj$crude_ci_upper * 100, 2)`%) and the age- and sex-adjusted updated prevalence of autism was `r sprintf("%.2f", round(aut_newprev_araucS_adj$adjusted_rate * 100, 2))`% (`r round(aut_newprev_araucS_adj$adjusted_ci_lower * 100, 2)`-`r round(aut_newprev_araucS_adj$adjusted_ci_upper * 100, 2)`%). The sample size here was `r format(aut_newprev_araucS_adj$sum_sample_pop_size, big.mark = ",", trim = TRUE)`, the same as the number of students resident in SSAS, because no individuals were added, only non-cases in the school data reallocated as cases based on the patient data linkage findings. For females, the adjusted updated prevalence was `r round(aut_newprev_araucS_adj_f$adjusted_rate * 100, 2)`% (`r round(aut_newprev_araucS_adj_f$adjusted_ci_lower * 100, 2)`-`r round(aut_newprev_araucS_adj_f$adjusted_ci_upper * 100, 2)`%) and for males it was `r round(aut_newprev_araucS_adj_m$adjusted_rate * 100, 2)`% (`r round(aut_newprev_araucS_adj_m$adjusted_ci_lower * 100, 2)`-`r round(aut_newprev_araucS_adj_m$adjusted_ci_upper * 100, 2)`%). This gave an updated male to female ratio of `r round(aut_newprev_araucS_adj_m$adjusted_rate / aut_newprev_araucS_adj_f$adjusted_rate, 2)` after data linkage, smaller than the ratio of `r sprintf("%.2f", round(school_matched_small %>% filter(sex_desc.school == "Male") %>% nrow() / school_matched_small %>% filter(sex_desc.school == "Female") %>% nrow(), 2))`:1 in the SSAS school data before linkage. Updated autism prevalence was highest among individuals aged 6-8 at `r round(aut_newprev_araucS_adj_6$adjusted_rate * 100, 2)`% (`r round(aut_newprev_araucS_adj_6$adjusted_ci_lower * 100, 2)`-`r round(aut_newprev_araucS_adj_6$adjusted_ci_upper * 100, 2)`%) and decreased with age, see Table \@ref(tab:adjNewprevAraucSAgecat). 
...
aut_prev_health_adj$new_rate <- numeric(nrow(aut_prev_health_adj)) # Fill with 0's
...
for(i in 1:nrow(aut_prev_health_adj)) {
  if(aut_prev_health_adj$health_service_name[i] == "Araucanía Sur") {
    ...
    aut_prev_health_adj$new_ci_lower[i] <- aut_prev_health_adj$new_rate[i] - 0.5 * max(adjusted_ci_width, prev_delta_ci_width)
    aut_prev_health_adj$new_ci_upper[i] <- aut_prev_health_adj$new_rate[i] + 0.5 * max(adjusted_ci_width, prev_delta_ci_width)
...
kbl(aut_newprev_health_adj.table,
    col.names = c("Health service", "Adjusted prevalence (95% CI)", "Adjusted updated prevalence (Maximal 95% CI)"),
    align = "lrr",
    booktabs = TRUE,
    format = "pandoc",
    linesep = c(""), # No extra white space
    caption = "Adjusted prevalence and adjusted updated prevalence of autism by health service in Chile. Adjusted prevalence is from school data only. Adjusted updated prevalence is from linkage of school data and patient data. Prevalence for Servicio de Salud Araucanía Sur (SSAS) was calculated directly from linkage results. Prevalence for other health services was calcuated by adding the adjusted prevalence delta to each health service's adjusted prevalence from the school data only. Adjusted prevalence has 95% gamma confidence intervals. The width of the adjusted updated prevalence confidence intervals is the maximum of the school data adjusted prevalence confidence intervals for each health serce, and the adjusted prevalence delta confidence interval, except for SSAS which has the 95% gamma confidence intervals found earlier.") %>%
  add_header_above(c(" " = 1, "Adjusted prevalence" = 2))
```

```{r newPrevAraucSsex}
# Get the adjusted prevalence by sex for Arauc Sud only. Have to group then filter because
# we don't have the age-grouped population of Arauc Sud alone
aut_prev_araucS_f <- chile_bayes_aut %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_f <- get_adjusted_prev(aut_prev_araucS_f, grouping_vars = c("health_service_name")) %>%
  mutate(sex_desc = "Female") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_araucS_m <- chile_bayes_aut %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_m <- get_adjusted_prev(aut_prev_araucS_m, grouping_vars = c("health_service_name")) %>% 
  mutate(sex_desc = "Male") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_sex_araucS_adj <- rbind(aut_prev_araucS_adj_m, aut_prev_araucS_adj_f) 


araucS_lower_prev_sex <- aut_prev_sex_araucS_adj %>%
  filter(health_service_name == "Araucanía Sur") %>%
  select(adjusted_rate) #%>% as.vector()
araucS_upper_prev_sex <- rbind(aut_newprev_araucS_adj_m, aut_newprev_araucS_adj_f) %>%
  select(adjusted_rate) #%>% as.vector()

# Do this by sex and by agecat

prev_delta_sex <- (araucS_upper_prev_sex - araucS_lower_prev_sex) %>% unlist()
### Need to make this not a list

#need to do f and m separately to take max for each
prev_delta_sex_ci_width_f <- max(aut_prev_sex_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", sex_desc == "Female") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_sex_araucS_adj, sex_desc == "Female") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_f %>%
                             select(adjusted_ci_upper) -
                             aut_newprev_araucS_adj_f %>%
                             select(adjusted_ci_lower))
prev_delta_sex_ci_width_m <- max(aut_prev_sex_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", sex_desc == "Male") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_sex_araucS_adj, sex_desc == "Male") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_m %>%
                             select(adjusted_ci_upper) -
                             aut_newprev_araucS_adj_m %>%
                             select(adjusted_ci_lower))

araucS_lower_sex <- aut_prev_sex_araucS_adj %>%
  filter(health_service_name == "Araucanía Sur") %>%
  select(adjusted_count) #%>% as.numeric()
araucS_upper_sex <- rbind(aut_newprev_araucS_adj_m, aut_newprev_araucS_adj_f) %>%
  select(adjusted_count) #%>% as.numeric()
# araucS_mean <- (araucS_upper + araucS_lower) / 2
# araucS_sd <- (araucS_upper - araucS_lower) / (2*1.96)

aut_prev_sex_araucS_adj$new_rate <- numeric(nrow(aut_prev_sex_araucS_adj)) # Fill with 0's
aut_prev_sex_araucS_adj$new_ci_lower <- numeric(nrow(aut_prev_sex_araucS_adj))
aut_prev_sex_araucS_adj$new_ci_upper <- numeric(nrow(aut_prev_sex_araucS_adj))
aut_prev_sex_araucS_adj$new_count <- numeric(nrow(aut_prev_sex_araucS_adj))

for(i in 1:nrow(aut_prev_sex_araucS_adj)) {
  # If the health service is AraucS, redistribute people
  if(aut_prev_sex_araucS_adj$health_service_name[i] == "Araucanía Sur" & 
                aut_prev_sex_araucS_adj$sex_desc[i] == "Female") {
    aut_prev_sex_araucS_adj$new_rate[i] <- araucS_upper_prev_sex[2,]
    aut_prev_sex_araucS_adj$new_ci_lower[i] <- aut_newprev_araucS_adj_f$adjusted_ci_lower
    aut_prev_sex_araucS_adj$new_ci_upper[i] <- aut_newprev_araucS_adj_f$adjusted_ci_upper
    aut_prev_sex_araucS_adj$new_count[i] <- araucS_upper_sex[2,]
  }
  if(aut_prev_sex_araucS_adj$health_service_name[i] == "Araucanía Sur" &
                aut_prev_sex_araucS_adj$sex_desc[i] == "Male") {
    aut_prev_sex_araucS_adj$new_rate[i] <- araucS_upper_prev_sex[1,]
    aut_prev_sex_araucS_adj$new_ci_lower[i] <- aut_newprev_araucS_adj_m$adjusted_ci_lower
    aut_prev_sex_araucS_adj$new_ci_upper[i] <- aut_newprev_araucS_adj_m$adjusted_ci_upper
    aut_prev_sex_araucS_adj$new_count[i] <- araucS_upper_sex[1,]
  }
  # For the other health services, add the delta for that sex and the wider CI
  if(aut_prev_sex_araucS_adj$health_service_name[i] != "Araucanía Sur" &
                aut_prev_sex_araucS_adj$sex_desc[i] == "Female") {
    adjusted_ci_width <- aut_prev_sex_araucS_adj$adjusted_ci_upper[i] - aut_prev_sex_araucS_adj$adjusted_ci_lower[i]
    aut_prev_sex_araucS_adj$new_rate[i] <- aut_prev_sex_araucS_adj$adjusted_rate[i] + prev_delta_sex[[2]]
    aut_prev_sex_araucS_adj$new_ci_lower[i] <- aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric - 0.5 * max(adjusted_ci_width, prev_delta_sex_ci_width_f)
    aut_prev_sex_araucS_adj$new_ci_upper[i] <- aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric + 0.5 * max(adjusted_ci_width, prev_delta_sex_ci_width_f)
    aut_prev_sex_araucS_adj$new_count[i] <- round(aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric * aut_prev_sex_araucS_adj$sum_sample_pop_size[i], 0)
  }
  if(aut_prev_sex_araucS_adj$health_service_name[i] != "Araucanía Sur" &
                aut_prev_sex_araucS_adj$sex_desc[i] == "Male") {
    adjusted_ci_width <- aut_prev_sex_araucS_adj$adjusted_ci_upper[i] - aut_prev_sex_araucS_adj$adjusted_ci_lower[i]
    aut_prev_sex_araucS_adj$new_rate[i] <- aut_prev_sex_araucS_adj$adjusted_rate[i] + prev_delta_sex[[1]]
    aut_prev_sex_araucS_adj$new_ci_lower[i] <- aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric - 0.5 * max(adjusted_ci_width, prev_delta_sex_ci_width_m)
    aut_prev_sex_araucS_adj$new_ci_upper[i] <- aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric + 0.5 * max(adjusted_ci_width, prev_delta_sex_ci_width_m)
    aut_prev_sex_araucS_adj$new_count[i] <- round(aut_prev_sex_araucS_adj$new_rate[i] %>% as.numeric * aut_prev_sex_araucS_adj$sum_sample_pop_size[i], 0)
  }
}
```
...
```{r newPrevAraucSage}
# Get the adjusted prevalence by age group for Arauc Sud only. Have to group then filter because
# we don't have the age-grouped population of Arauc Sud alone
aut_prev_araucS_6 <- chile_bayes_aut %>%
  filter(age_cat_name == "6-8") %>%
  get_grouped_prev(stdpop = chile_stdpop_6, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_6 <- get_adjusted_prev(aut_prev_araucS_6, grouping_vars = c("health_service_name")) %>%
  mutate(age_cat_name = "6-8") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_araucS_9 <- chile_bayes_aut %>%
  filter(age_cat_name == "9-11") %>%
  get_grouped_prev(stdpop = chile_stdpop_9, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_9 <- get_adjusted_prev(aut_prev_araucS_9, grouping_vars = c("health_service_name")) %>% 
  mutate(age_cat_name = "9-11") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_araucS_12 <- chile_bayes_aut %>%
  filter(age_cat_name == "12-14") %>%
  get_grouped_prev(stdpop = chile_stdpop_12, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_12 <- get_adjusted_prev(aut_prev_araucS_12, grouping_vars = c("health_service_name")) %>% 
  mutate(age_cat_name = "12-14") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_araucS_15 <- chile_bayes_aut %>%
  filter(age_cat_name == "15-18") %>%
  get_grouped_prev(stdpop = chile_stdpop_15, 
                   grouping_vars = c("health_service_name", "age_june30", "sex", "autism"), disease = "autism") #%>%
  #filter(health_service_name == "Araucanía Sur")
aut_prev_araucS_adj_15 <- get_adjusted_prev(aut_prev_araucS_15, grouping_vars = c("health_service_name")) %>% 
  mutate(age_cat_name = "15-18") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_age_araucS_adj <- rbind(aut_prev_araucS_adj_6, aut_prev_araucS_adj_9, aut_prev_araucS_adj_12, aut_prev_araucS_adj_15) 


araucS_lower_prev_age <- aut_prev_age_araucS_adj %>%
  filter(health_service_name == "Araucanía Sur") %>%
  select(adjusted_rate) #%>% as.vector()
araucS_upper_prev_age <- rbind(aut_newprev_araucS_adj_6, aut_newprev_araucS_adj_9, 
                               aut_newprev_araucS_adj_12, aut_newprev_araucS_adj_15) %>%
  select(adjusted_rate) #%>% as.vector()

# Do this by sex and by agecat

prev_delta_age <- (araucS_upper_prev_age - araucS_lower_prev_age) %>% unlist()
### Need to make this not a list

#need to do f and m separately to take max for each
prev_delta_age_ci_width_6 <- max(aut_prev_age_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", age_cat_name == "6-8") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_age_araucS_adj, age_cat_name == "6-8") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_6 %>%
                             select(adjusted_ci_upper) -
                             aut_newprev_araucS_adj_6 %>%
                             select(adjusted_ci_lower))
prev_delta_age_ci_width_9 <- max(aut_prev_age_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", age_cat_name == "9-11") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_age_araucS_adj, age_cat_name == "9-11") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_9 %>%
                             select(adjusted_ci_upper) -
                             aut_newprev_araucS_adj_9 %>%
                             select(adjusted_ci_lower))
prev_delta_age_ci_width_12 <- max(aut_prev_age_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", age_cat_name == "12-14") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_age_araucS_adj, age_cat_name == "12-14") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_12 %>%
                             select(adjusted_ci_upper) -
                             aut_newprev_araucS_adj_12 %>%
                             select(adjusted_ci_lower))
prev_delta_age_ci_width_15 <- max(aut_prev_age_araucS_adj %>%
                             filter(health_service_name == "Araucanía Sur", age_cat_name == "15-18") %>%
                             select(adjusted_ci_upper) - 
                             filter(aut_prev_age_araucS_adj, age_cat_name == "15-18") %>% 
                             filter(health_service_name == "Araucanía Sur") %>%
                             select(adjusted_ci_lower),
                           aut_newprev_araucS_adj_15 %>%
                             select(adjusted_ci_upper) -
...
araucS_lower_age <- aut_prev_age_araucS_adj %>%
  filter(health_service_name == "Araucanía Sur") %>%
  select(adjusted_count) #%>% as.numeric()
araucS_upper_age <- rbind(aut_newprev_araucS_adj_6, aut_newprev_araucS_adj_9,
                          aut_newprev_araucS_adj_12, aut_newprev_araucS_adj_15) %>%
  select(adjusted_count) #%>% as.numeric()
# araucS_mean <- (araucS_upper + araucS_lower) / 2
# araucS_sd <- (araucS_upper - araucS_lower) / (2*1.96)
...
The adjusted prevalence delta for SSAS was `r round(prev_delta * 100, 2)`% (`r round((prev_delta - 0.5*prev_delta_ci_width) * 100, 2)`, `r round((prev_delta + 0.5*prev_delta_ci_width) * 100, 2)`%) and represents the unmet need of children with autism in SSAS that did not access SEED for it. Table \@ref(tab:newPrevAraucS) shows the projection of adjusted updated prevalence from the data linkage for SSAS onto the other health services. The patterns of prevalence across health services were retained and Ñuble has the highest adjusted updated prevalence at `r round(aut_prev_health_adj %>% filter(health_service_code == 29) %>% select(new_rate) %>% as.numeric() * 100, 2)`% (`r round(aut_prev_health_adj %>% filter(health_service_code == 29) %>% select(new_ci_lower) %>% as.numeric() * 100, 2)`, `r sprintf("%.2f", round(aut_prev_health_adj %>% filter(health_service_code == 29) %>% select(new_ci_upper) %>% as.numeric() * 100, 2))`%). Adding the adjusted prevalence delta to the national adjusted autism prevalence found from the school data alone gave national adjusted updated autism prevalence of `r round((aut_prev_adj$adjusted_rate + prev_delta) * 100, 2)`%. This in turn gave a posited `r format(round((aut_prev_adj$adjusted_rate + prev_delta) * nrow(chile_bayes), 0), big.mark = ",", trim = TRUE)` children aged 6-18 with autism in Chile and corresponds to an estimated unmet need of `r format(round((prev_delta) * nrow(chile_bayes), 0) , big.mark = ",", trim = TRUE)` children that did not access SEED for autism (posited count and unmet need are age- and sex-adjusted).
...
Bayesian prevalence projections by health service with common national autism prevalence prior are shown in Figure \@ref(fig:bayesConstantPrior). The differences in adjusted sample prevalence across health services are evident in the red bands and the posterior predictive distributions have been pulled towards the common prior. For regions like Ñuble which have adjusted sample prevalence in the school data higher than the national adjusted prevalence, these posterior densities are not plausible.
...
In Figure \@ref(fig:bayesUniformPrior), the posterior distributions were reflective of their uniform priors with posterior credible intervals slightly within the prior bounds (Table \@ref(tab:newPrevAraucS)). These predictive densities show a considerable departure from the adjusted sample prevalence from the school data and provide a window within which the true prevalence of recorded autism in each health service would plausibly fall.

```{r bayesUniformPriorSex, include = FALSE}

# Original delta
aut_prev_sex_adj$new_rate <- aut_prev_sex_adj$adjusted_rate + prev_delta
aut_prev_sex_adj$new_count <- round(aut_prev_sex_adj$new_rate * aut_prev_sex_adj$sum_sample_pop_size, 0)

# OR
# Sex-specific delta
aut_prev_sex_adj$new_rate <- aut_prev_sex_adj$adjusted_rate + unlist(prev_delta_sex)
aut_prev_sex_adj$new_count <- round(aut_prev_sex_adj$new_rate * aut_prev_sex_adj$sum_sample_pop_size, 0)


nFeat <- nrow(aut_prev_sex_adj)
FeatNames <- sort(unique(aut_prev_sex_adj$sex_desc), decreasing = TRUE)

# Uniform prior between lower bound and delta for all school data by rurality
rand_model_uniform <- "model {
  for(i in 1:nFeat) { # For each category in the feature grouping
    theta[i] ~ dunif(theta_lower[i], theta_upper[i])     #dbeta(theta_a[i], theta_b[i])
    disease_sample[i] ~ dbin(theta[i], nObs[i])
...
# Define uniform prior
theta_lower <- aut_prev_sex_adj$adjusted_rate
theta_upper <- aut_prev_sex_adj$new_rate
...
rand_data <- list(theta_lower = theta_lower,
                  theta_upper = theta_upper,
                  nObs = aut_prev_sex_adj$sum_sample_pop_size,
                  aut_sample = aut_prev_sex_adj$adjusted_count,
                  nFeat = nFeat)
rand_jag <- jags.model(textConnection(rand_model_uniform),
                       data = rand_data,
                       inits = rand_ini,
...
aut_prev_sex_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                      rownames = "Iteration") %>%
    select(c("Iteration", contains("theta["))) %>%
    pivot_longer(cols = contains("theta["),
                 names_to = "sex_desc",
                 values_to = "predicted_prev") %>%
    mutate(Feat_names = factor(sex_desc,
                               levels = c(paste0("theta[",1:nFeat,"]")),
                               labels = FeatNames)) %>%
    select(Iteration, Feat_names, predicted_prev) %>%
...
# Plot posterior densities
aut_prev_sex_post_new_ci <- aut_prev_sex_post_new %>%
  group_by(sex_desc) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))
# aut_prev_sex_post_new_ci is the ci to report
  
ggplot() +
  geom_density(data = aut_prev_sex_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_sex_post_new_ci, aes(xintercept = post_lower*100), 
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_sex_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_sex_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_sex_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  facet_wrap(~sex_desc) +
  labs(title = "Posterior predicitve distribution for sex specific uniform priors",
       x = "Prevalence %",
       y = "Density")

round(aut_prev_sex_adj$new_rate*100, 2)
aut_prev_sex_post_new_ci

```

```{r bayesUniformPriorAgegroup, include = FALSE}

aut_prev_age_adj <- rbind(aut_newprev_araucS_adj_12, aut_newprev_araucS_adj_15, 
                               aut_newprev_araucS_adj_6, aut_newprev_araucS_adj_9)

# Original delta
aut_prev_age_adj$new_rate <- aut_prev_age_adj$adjusted_rate + prev_delta
aut_prev_age_adj$new_count <- round(aut_prev_age_adj$new_rate * aut_prev_age_adj$sum_sample_pop_size, 0)

# OR
# Sex-specific delta
aut_prev_age_adj$new_rate <- aut_prev_age_adj$adjusted_rate + unlist(prev_delta_age)
aut_prev_age_adj$new_count <- round(aut_prev_age_adj$new_rate * aut_prev_age_adj$sum_sample_pop_size, 0)


nFeat <- nrow(aut_prev_age_adj)
FeatNames <- sort(unique(aut_prev_age_adj$age_cat_name))#, decreasing = TRUE) 
...
# Define uniform prior
theta_lower <- aut_prev_age_adj$adjusted_rate
theta_upper <- aut_prev_age_adj$new_rate
...
rand_data <- list(theta_lower = theta_lower,
                  theta_upper = theta_upper,
                  nObs = aut_prev_age_adj$sum_sample_pop_size,
                  aut_sample = aut_prev_age_adj$adjusted_count,
                  nFeat = nFeat)
rand_jag <- jags.model(textConnection(rand_model_uniform),
                       data = rand_data,
                       inits = rand_ini,
...
aut_prev_age_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                      rownames = "Iteration") %>%
    select(c("Iteration", contains("theta["))) %>%
    pivot_longer(cols = contains("theta["),
                 names_to = "age_cat_name",
                 values_to = "predicted_prev") %>%
    mutate(Feat_names = factor(age_cat_name,
                               levels = c(paste0("theta[",1:nFeat,"]")),
                               labels = FeatNames)) %>%
    select(Iteration, Feat_names, predicted_prev) %>%
  rename(age_cat_name = Feat_names) #%>%
  #merge(health_service_lookup, by = "health_service_code") %>%
  #rename(health_service_name_long = health_service_name,
  #       health_service_name = health_service_name_short)
...
# Plot posterior densities
aut_prev_age_post_new_ci <- aut_prev_age_post_new %>%
  group_by(age_cat_name) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))
# aut_prev_sex_post_new_ci is the ci to report
...
ggplot() +
  geom_density(data = aut_prev_age_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_age_post_new_ci, aes(xintercept = post_lower*100), 
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_age_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_age_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_age_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  facet_wrap(~age_cat_name) +
  labs(title = "Posterior predicitve distribution for age specific uniform priors",
       x = "Prevalence %",
       y = "Density")

aut_prev_age_adj$age_cat_name
round(aut_prev_age_adj$new_rate*100, 2)
aut_prev_age_post_new_ci

```


```{r bayesUniformPriorRural, include = FALSE}

aut_prev_rural_adj$new_rate <- aut_prev_rural_adj$adjusted_rate + prev_delta
aut_prev_rural_adj$new_count <- round(aut_prev_rural_adj$new_rate * aut_prev_rural_adj$sum_sample_pop_size, 0)

nFeat <- nrow(aut_prev_rural_adj)
FeatNames <- sort(unique(aut_prev_rural_adj$school_rurality))
...
# Define uniform prior
theta_lower <- aut_prev_rural_adj$adjusted_rate
theta_upper <- aut_prev_rural_adj$new_rate
...
aut_prev_rural_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                      rownames = "Iteration") %>%
    select(c("Iteration", contains("theta["))) %>%
    pivot_longer(cols = contains("theta["),
                 names_to = "school_rurality",
                 values_to = "predicted_prev") %>%
    mutate(Feat_names = factor(school_rurality,
                               levels = c(paste0("theta[",1:nFeat,"]")),
                               labels = FeatNames)) %>%
    select(Iteration, Feat_names, predicted_prev) %>%
...
# Plot posterior densities
aut_prev_rural_post_new_ci <- aut_prev_rural_post_new %>%
  group_by(school_rurality) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))
  
ggplot() +
  geom_density(data = aut_prev_rural_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_rural_post_new_ci, aes(xintercept = post_lower*100), 
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_rural_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_rural_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_rural_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  facet_wrap(~school_rurality) +
  labs(title = "Posterior predicitve distribution for rurality specific uniform priors",
       x = "Prevalence %",
       y = "Density")

```

```{r bayesUniformPriorEthnicall, include = FALSE}

# Need to recalculate the ethnic prevalences using all data not just Mapuche regions and 
# with Aymara separated out

aut_prev_ethnicall <- get_grouped_prev(x = chile_bayes_aut, stdpop = chile_stdpop, 
                                    grouping_vars = c("ethnic_3_group",
                                                      "age_june30", 
                                                      "age_cat_name",
                                                      "sex",
                                                      "sex_desc",
...
aut_prev_ethnicall_adj <- get_adjusted_prev(aut_prev_ethnicall, 
                                         grouping_vars = "ethnic_3_group") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_ethnicall_adj$new_rate <- aut_prev_ethnicall_adj$adjusted_rate + prev_delta
aut_prev_ethnicall_adj$new_count <- round(aut_prev_ethnicall_adj$new_rate * aut_prev_ethnicall_adj$sum_sample_pop_size, 0)

nFeat <- nrow(aut_prev_ethnicall_adj)
FeatNames <- sort(unique(aut_prev_ethnicall_adj$ethnic_3_group))
...
# Define uniform prior
theta_lower <- aut_prev_ethnicall_adj$adjusted_rate
theta_upper <- aut_prev_ethnicall_adj$new_rate
...
rand_data <- list(theta_lower = theta_lower,
                  theta_upper = theta_upper,
                  nObs = aut_prev_ethnicall_adj$sum_sample_pop_size,
                  aut_sample = aut_prev_ethnicall_adj$adjusted_count,
                  nFeat = nFeat)
rand_jag <- jags.model(textConnection(rand_model_uniform),
                       data = rand_data,
                       inits = rand_ini,
...
aut_prev_ethnicall_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                      rownames = "Iteration") %>%
    select(c("Iteration", contains("theta["))) %>%
    pivot_longer(cols = contains("theta["),
                 names_to = "ethnic_3_group",
                 values_to = "predicted_prev") %>%
    mutate(Feat_names = factor(ethnic_3_group,
                               levels = c(paste0("theta[",1:nFeat,"]")),
                               labels = FeatNames)) %>%
    select(Iteration, Feat_names, predicted_prev) %>%
  rename(ethnic_3_group = Feat_names) #%>%
  #merge(health_service_lookup, by = "health_service_code") %>%
  #rename(health_service_name_long = health_service_name,
  #       health_service_name = health_service_name_short)
...
# Plot posterior densities
aut_prev_ethnicall_post_new_ci <- aut_prev_ethnicall_post_new %>%
  group_by(ethnic_3_group) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))
  
ggplot() +
  geom_density(data = aut_prev_ethnicall_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_ethnicall_post_new_ci, aes(xintercept = post_lower*100), 
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_ethnicall_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_ethnicall_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_ethnicall_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  facet_wrap(~ethnic_3_group) +
  labs(title = "Posterior predicitve distribution for ethnicity specific uniform priors",
       x = "Prevalence %",
       y = "Density")

```

```{r bayesUniformPriorImmi, include = FALSE}

# By immigration status

aut_prev_immi <- get_grouped_prev(x = chile_bayes_aut, stdpop = chile_stdpop, 
                                    grouping_vars = c("immigration",
                                                      "age_june30", 
                                                      "age_cat_name",
                                                      "sex",
                                                      "sex_desc",
...
aut_prev_immi_adj <- get_adjusted_prev(aut_prev_immi, 
                                         grouping_vars = "immigration") %>%
  mutate(crude_ci_lower = ifelse(crude_ci_lower < 0, 0, crude_ci_lower),
         adjusted_ci_lower = ifelse(adjusted_ci_lower < 0, 0, adjusted_ci_lower))

aut_prev_immi_adj$new_rate <- aut_prev_immi_adj$adjusted_rate + prev_delta
aut_prev_immi_adj$new_count <- round(aut_prev_immi_adj$new_rate * aut_prev_immi_adj$sum_sample_pop_size, 0)

nFeat <- nrow(aut_prev_immi_adj)
FeatNames <- sort(unique(aut_prev_immi_adj$immigration))
...
# Define uniform prior
theta_lower <- aut_prev_immi_adj$adjusted_rate
theta_upper <- aut_prev_immi_adj$new_rate
...
rand_data <- list(theta_lower = theta_lower,
                  theta_upper = theta_upper,
                  nObs = aut_prev_immi_adj$sum_sample_pop_size,
                  aut_sample = aut_prev_immi_adj$adjusted_count,
                  nFeat = nFeat)
rand_jag <- jags.model(textConnection(rand_model_uniform),
                       data = rand_data,
                       inits = rand_ini,
...
aut_prev_immi_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                      rownames = "Iteration") %>%
    select(c("Iteration", contains("theta["))) %>%
    pivot_longer(cols = contains("theta["),
                 names_to = "immigration",
                 values_to = "predicted_prev") %>%
    mutate(Feat_names = factor(immigration,
                               levels = c(paste0("theta[",1:nFeat,"]")),
                               labels = FeatNames)) %>%
    select(Iteration, Feat_names, predicted_prev) %>%
  rename(immigration = Feat_names) #%>%
  #merge(health_service_lookup, by = "health_service_code") %>%
  #rename(health_service_name_long = health_service_name,
  #       health_service_name = health_service_name_short)
...
# Plot posterior densities
aut_prev_immi_post_new_ci <- aut_prev_immi_post_new %>%
  group_by(immigration) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))
  
ggplot() +
  geom_density(data = aut_prev_immi_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_immi_post_new_ci, aes(xintercept = post_lower*100), 
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_immi_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_immi_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_immi_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  facet_wrap(~immigration) +
  labs(title = "Posterior predicitve distribution for immigration status specific uniform priors",
       x = "Prevalence %",
       y = "Density")

```

```{r bayesUniformPriorCommon, include = FALSE}

# Common effects model to do all data together

aut_prev_adj$new_rate <- aut_prev_adj$adjusted_rate + prev_delta
aut_prev_adj$new_count <- round(aut_prev_adj$new_rate * aut_prev_adj$sum_sample_pop_size, 0)

nFeat <- nrow(aut_prev_adj)
FeatNames <- sort(unique(aut_prev_adj$ethnic_3_group))

# Uniform prior between lower bound and delta for all school data

common_model <- "model {
  theta ~ dunif(theta_lower, theta_upper) # Prior
  disease_sample ~ dbin(theta, nObs) # Prevalence in sample data
  disease_pred ~ dbin(theta, nObs) # Predicted prevalence in new sample of same size
}"

pars <- c("theta", "disease_sample", "disease_pred")

# Define uniform prior
theta_lower <- aut_prev_adj$adjusted_rate
theta_upper <- aut_prev_adj$new_rate
...
rand_data <- list(theta_lower = theta_lower,
                  theta_upper = theta_upper,
                  nObs = aut_prev_adj$sum_sample_pop_size,
                  aut_sample = aut_prev_adj$adjusted_count) #,
                  #nFeat = nFeat)
rand_jag <- jags.model(textConnection(common_model),
                       data = rand_data,
                       inits = rand_ini,
                       n.chains = 2,
                       quiet = TRUE)
...
convergence_check <- FALSE
if(convergence_check) {
  print(mcmc_trace(rand_sam, paste0("theta")))
  print(mcmc_trace(rand_sam, paste0("disease_pred")))
  rand_summ <- summary(subset_draws(as_draws(rand_sam), pars),
                           ~quantile(.x, probs=c(0.025, 0.5, 0.975)),
                           ~mcse_quantile(.x, probs=c(0.025, 0.5, 0.975)),
                           "rhat") %>%
...
aut_prev_post_new <- as_tibble(as_draws_matrix(rand_sam), 
                                     rownames = "Iteration") %>%
   select(c("Iteration", contains("theta"))) %>%
   pivot_longer(cols = contains("theta"),
                names_to = "theta",
...
# Plot posterior densities
aut_prev_post_new_ci <- aut_prev_post_new %>%
  #group_by(ethnic_3_group) %>%
  summarise(post_lower = quantile(predicted_prev, 0.025),
            post_upper = quantile(predicted_prev, 0.975))

ggplot() +
  geom_density(data = aut_prev_post_new, aes(x = predicted_prev*100)) +
  geom_vline(data = aut_prev_post_new_ci, aes(xintercept = post_lower*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_post_new_ci, aes(xintercept = post_upper*100),
             color = "blue", linetype = "dotted") +
  geom_vline(data = aut_prev_adj, aes(xintercept = adjusted_ci_lower*100),
             color = "red", linetype = "dashed") +
  geom_vline(data = aut_prev_adj, aes(xintercept = adjusted_ci_upper*100),
             color = "red", linetype = "dashed") +
  #facet_wrap(~ethnic_3_group) +
  labs(title = "Posterior predicitve distribution for uniform priors",
       x = "Prevalence %",
       y = "Density")
...
Prevalence of autism and ADHD is lower among students of the Mapuche Indigenous group than among non-Indigenous students. For ADHD, prevalence is even lower among member of other Indigenous groups. For autism, prevalence in other Indigenous groups is lower than for non-Indigenous groups but higher than for Mapuche. This agrees with Lindblom’s findings that Indigenous children with autism in Canada’s British Colombia are underrepresented in school data (67). However it is important to note here that people Indigenous to different regions are not necessarily comparable and very little research has been done on autism and ADHD in Indigenous people in Latin America. 

Autism and ADHD prevalence is higher in students at rural schools that urban schools which is somewhat surprising as urban areas generally have more resources to diagnose and provide adjustments for these conditions, but may be the result of successful implementation of policy interventions described by Núñez and Manzano that aimed to improve healthcare for disadvantaged areas and population groups (6).
...
This investigation has furthered the understanding of autism and ADHD prevalence in Chile. Autism prevalence is at least `r round(aut_prev_adj$adjusted_rate * 100, 2)`% (`r round(aut_prev_sex_adj$adjusted_rate[2] * 100, 2)`% in females,`r round(aut_prev_sex_adj$adjusted_rate[1] * 100, 2)`% in males and male to female ratio of `r sprintf("%.2f", round(aut_prev_sex_adj$adjusted_rate[1] / aut_prev_sex_adj$adjusted_rate[2], 2))`:1) and ADHD prevalence is at least `r round(adhd_prev_adj$adjusted_rate * 100, 2)`% (`r round(adhd_prev_sex_adj$adjusted_rate[2] * 100, 2)`% in females, `r round(adhd_prev_sex_adj$adjusted_rate[1] * 100, 2)`% in males and male to female ratio of `r round(adhd_prev_sex_adj$adjusted_rate[1] / adhd_prev_sex_adj$adjusted_rate[2], 2)`:1). Both autism and ADHD are more prevalent in non-Indigenous students and students at rural schools, and neither have a clear difference in prevalence by SES. In Servicio de Salud Araucanía Sur, age, ethnicity and commune of residence are important components for explaining the variance in data on patients with autism. School data and patient data on individuals with autism in SSAS were successfully linked. From linkage, a more accurate estimate of the prevalence of autism in this region is `r round(aut_newprev_araucS_adj$adjusted_rate * 100, 2)`% (male to female ratio of `r round(aut_newprev_araucS_adj_m$adjusted_rate / aut_newprev_araucS_adj_f$adjusted_rate, 2)`:1) and the estimated prevalence in Chile is `r round((aut_prev_adj$adjusted_rate + prev_delta) * 100, 2)`%. The unmet need of students with autism that did not receive SEED funding for support with it is `r format(patients_matched_small_dedup %>% filter(matched == 0) %>% nrow(), big.mark = ",", trim = TRUE)` students in SSAS and a posited `r format(round((prev_delta) * nrow(chile_bayes), 0) , big.mark = ",", trim = TRUE)`across Chile. Linkage also revealed females are likely underrepresented in school autism diagnoses. Bayesian prevalence projection extended prevalence projections across health services and found plausible prevalence bounds for each health service.

# Supplementary materials

```{r prevAgecatAutTable, fig.pos='H'}
# Autism
aut_prev.agecat <- get_grouped_prev_plot(x = chile_bayes_aut, 
                                          grouping_vars = c("age_cat_name", "autism"),
                                          disease = "autism") %>%
...
```{r prevAgecatAdhdTable, fig.pos='H'}
# ADHD
adhd_prev.agecat <- get_grouped_prev_plot(x = chile_bayes_adhd, 
                                          grouping_vars = c("age_cat_name", "adhd"),
                                          disease = "adhd") %>%
...
```{r prevAgecatSexAutTable, fig.pos='H'}
# Autism
aut_prev.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_aut, 
...
```{r prevAgecatSexAdhdTable, fig.pos='H'}
# ADHD
adhd_prev.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_adhd, 
...
```{r prevAgecatHealthAutTable, fig.pos='H'}
# Autism
...
```{r prevAgecatHealthAdhdTable, fig.pos='H'}
# ADHD
...
```{r prevAgecatHealthAutPlot, fig.cap = "Crude prevalence of autism in school data by health service, age band and sex. Bars show 95% normal confidence intervals.", fig.height = 10, fig.width = 8.5, fig.pos='H'}
# Autism
aut_prev_health.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_aut, 
                                          grouping_vars = c("health_service_name" ,"age_cat_name", "sex_desc", "autism"),
                                          disease = "autism") %>%
...
```{r prevAgecatHealthAdhdPlot, fig.cap = "Crude prevalence of ADHD in school data by health service, age band and sex. Bars show 95% normal confidence intervals.", fig.height = 10, fig.width = 8.5, fig.pos='H'}
# ADHD
adhd_prev_health.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_adhd, 
                                          grouping_vars = c("health_service_name", "age_cat_name", "sex_desc", "adhd"),
                                          disease = "adhd") %>%
...
```{r prevAgecatEconAAutTable, fig.pos='H'}
# Autism
...
```{r prevAgecatEconAAdhdTable, fig.pos='H'}
# ADHD
...
```{r prevAgecatEconAAutPlot, fig.cap = "Crude prevalence of autism in school data by student's monthly school fee (Peso), age band and sex. Bars show 95% normal confidence intervals.", fig.height=6, fig.pos='H'}
# Autism
aut_prev_econA.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_aut,
                                                    grouping_vars = c("school_fee", "age_cat_name", "sex_desc", "autism"),
                                                    disease = "autism") %>%
...
```{r prevAgecatEconAAdhdPlot, fig.cap = "Crude prevalence of ADHD in school data by student's monthly school fee (Peso), age band and sex. Bars show 95% normal confidence intervals.", fig.height=6, fig.pos='H'}
# ADHD
adhd_prev_econA.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_adhd,
                                                    grouping_vars = c("school_fee", "age_cat_name", "sex_desc", "adhd"),
                                                    disease = "adhd") %>%
...
```{r prevAgecatEthnicAutTable, fig.pos='H'}
# Autism
...
```{r prevAgecatEthnicAdhdTable, fig.pos='H'}
# ADHD
...
```{r prevAgecatEthnicAutPlot, fig.cap = "Crude prevalence of autism in school data by ethnicity, age band and sex. Bars show 95% normal confidence intervals. Regions with high Mapuche populations only.", fig.height=3, fig.pos='H'}
# Autism
aut_prev_ethnic.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_aut_ethnic,
                                                    grouping_vars = c("ethnic_2_group", "age_cat_name", "sex_desc", "autism"),
                                                    disease = "autism") %>%
...
```{r prevAgecatEthnicAdhdPlot, fig.cap = "Crude prevalence of ADHD in school data by ethnicity, age band and sex. Bars show 95% normal confidence intervals. Regions with high Mapuche populations only.", fig.height=3, fig.pos='H'}
# ADHD
adhd_prev_ethnic.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_adhd_ethnic,
                                                    grouping_vars = c("ethnic_2_group", "age_cat_name", "sex_desc", "adhd"),
                                                    disease = "adhd") %>%
...
```{r prevAgecatRuralAutTable, fig.pos='H'}
# Autism
...
```{r prevAgecatRuralAdhdTable, fig.pos='H'}
# ADHD
...
```{r prevAgecatRuralAutPlot, fig.cap = "Crude prevalence of autism in school data by school's rurality, age band and sex. Bars show 95% normal confidence intervals.", fig.height=3, fig.pos='H'}
# Autism
aut_prev_rural.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_aut,
                                                    grouping_vars = c("school_rurality", "age_cat_name", "sex_desc", "autism"),
                                                    disease = "autism") %>%
...
```{r prevAgecatRuralAdhdPlot, fig.cap="Crude prevalence of ADHD in school data by school's rurality, age band and sex. Bars show 95% normal confidence intervals.", fig.height=3, fig.pos='H'}
# ADHD
adhd_prev_rural.agecat_sex <- get_grouped_prev_plot(x = chile_bayes_adhd,
                                                    grouping_vars = c("school_rurality", "age_cat_name", "sex_desc", "adhd"),
                                                    disease = "adhd") %>%
...
```{r kolmogorovSchoolSex, fig.cap="a) Difference in frequency of sexes among school records that matched to patient records and school records that did not match. b) Density of Kolmogorov-Smirnov p-values for 2000 permutations of matched status for school records by sex with observed p-value shown in red.", fig.height=8, fig.pos='H'}
...
```{r kolmogorovSchoolCommune, fig.cap="a) Difference in frequency of communes of residence among school records that matched to patient records and school records that did not match. b) Density of Kolmogorov-Smirnov p-values for 2000 permutations of matched status for school records by commune of residence with observed p-value shown in red.", fig.height=8, fig.pos='H'}
...
```{r kolmogorovSchoolSes, fig.cap="a) Difference in frequency of proxy SES among school records that matched to patient records and school records that did not match. b) Density of Kolmogorov-Smirnov p-values for 2000 permutations of matched status for school records by proxy SES with observed p-value shown in red.", fig.height=8, fig.pos='H'}
...
17.	Centro de Innovación en Educación Fundación Chile. Análisis de la implementación de los programas de integración escolar (PIE) en establecimientos que han incorporado estudiantes con necesidades educativas especiales transitorias (NEET) [Internet]. 2013 [cited 2023 Jul 18]. Available from: https://bibliotecadigital.mineduc.cl/bitstream/handle/20.500.12365/18386/EV13-0012.pdf?sequence=1
...
57.	Roman-Urrestarazu A, Yang JC, van Kessel R, Warrier V, Dumas G, Jongsma H, et al. Autism incidence and spatial analysis in more than 7 million pupils in English schools: a retrospective, longitudinal, school registry study. Lancet Child Adolesc Health. 2022 Dec 1;6(12):857–68. 
...
59.	Instituto Nacional de Estadisticas. Default. [cited 2023 Jul 5]. Proyecciones de Población. Available from: http://www.ine.gob.cl/estadisticas/sociales/seguridad-publica-y-justicia/estadisticas-policiales-y-judiciales/proyecciones-de-población
...
```{r setupAim1, echo = TRUE, eval = FALSE}
library(janitor)
library(gridExtra)
library(readxl)
library(writexl)
