...
school.table <- chile_bayes %>%
  ...
  rbind(chile_bayes %>%
        ...
        chile_bayes %>%
          group_by(school_fee) %>% 
          summarise(variable = "Proxy SES (monthly school fee)",
                    count = n(), 
                    perc = sprintf("%.2f", round(count/nrow(chile_bayes)*100, 2))) %>%
...
school <- rbind(school_temp, c("2023-06-01", NA, NA, NA, NA))
```

```{r SSASschoolTable}
ssas_school.table <- school_temp %>%
  group_by(sex_desc) %>% 
  summarise(variable = "Sex",
            count = n(), 
            perc = sprintf("%.2f", round(count/nrow(school_temp)*100, 2))) %>%
  rename(values = sex_desc) %>%
  rbind(school_temp %>%
...
In the school records, `r format(school_all %>% nrow(), big.mark = ",", trim = TRUE)` students live in SSAS health service catchment, of which `r nrow(school)-1` (`r round((nrow(school)-1) / nrow(school_all) * 100, 2)`%) have autism. Among the students with autism, shown in \@ref(tab:SSASschoolTable), there are many more males than females, Temuco is the most common commune of residence and most students have a proxy SES of 1. 
...
```{r SSASpatientsTable}
ssas_patients.table <- patients %>%
  group_by(sex_desc) %>% 
  summarise(variable = "Sex",
            count = n(), 
            perc = sprintf("%.2f", round(count/nrow(patients)*100, 2))) %>%
  rename(values = sex_desc) %>%
  rbind(patients %>%
          group_by(student_commune_name) %>% 
          summarise(variable = "Commune",
