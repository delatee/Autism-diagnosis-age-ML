...
school_patients <- patients_matched_small_dedup %>% 
  ...
  select(id, dob, sex_desc, student_commune_name, autism, ses_status) %>%
  mutate(dataset = "patients") %>%
  rbind(school_all %>% mutate(dataset = "school", id = as.character(id)))



# Get deduped patients ie without the 2nd commune ones
# Add that to the original school data with and without autism
# Do age and sex adjustment
