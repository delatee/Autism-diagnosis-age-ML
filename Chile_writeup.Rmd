...
get_adjusted_prev <- function(x, grouping_vars) {
  ...
  summarise(sum_sample_pop_size = sum(sample_pop_size),
            ...
            adjusted_count = round(adjusted_rate * sum_sample_pop_size, 0), 
              # had to fudge this to get MCMC to run bc it needs integers
            ...
            adjusted_ci_upper = (var + w_M^2) / (2*(adjusted_rate + w_M)) * 
                                         qchisq(p = 1-0.05/2, 
                                                df = 2*(adjusted_rate+w_M)^2 / (var+w_M^2)),
...
school.table <- chile_bayes %>%
  ...
  rbind(chile_bayes %>%
        ...
        chile_bayes %>%
          ...
          mutate(values = factor(ifelse(values == 0, "No SEED", "Yes SEED"), levels = c("Yes SEED", "No SEED"))) %>%
          arrange(values),
        chile_bayes %>%
          group_by(autism) %>% 
          ...
          mutate(values = factor(ifelse(values == 0, "No autism", "Yes autism"), levels = c("Yes autism", "No autism"))) %>%
          arrange(values),
        chile_bayes %>%
          group_by(adhd) %>% 
          summarise(variable = "ADHD",
                    ...
                    perc = sprintf("%.2f", round(count/nrow(chile_bayes)*100, 2))) %>%
          rename(values = adhd) %>%
          mutate(values = factor(ifelse(values == 0, "No ADHD", "Yes ADHD"), levels = c("Yes ADHD", "No ADHD"))) %>%
          arrange(values)) %>%
...
Autism and ADHD are both significantly more prevalent for students at rural schools than urban schools, as shown in Tables \@ref(tab:adjPrevRuralAutTable) and \@ref(tab:adjPrevRuralAdhdTable). See also Supplementary Tables \@ref(tab:prevAgecatRuralAutTable) and \@ref(tab:prevAgecatRuralAdhdTable) and Supplementary Figures \@ref(fig:prevAgecatRuralAutPlot), \@ref(fig:prevAgeRuralAutPlot), \@ref(fig:prevAgecatRuralAdhdPlot) and \@ref(fig:prevAgeRuralAdhdPlot).
