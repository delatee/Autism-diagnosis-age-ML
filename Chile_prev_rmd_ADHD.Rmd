---
title: "Bayesian prevalence analysis of ADHD prevalence in Chile"
ADHDhor: "Adele Tyson"
date: "`r Sys.Date()`"
output: pdf_document
---
...
chile_merged <- chile_merged_raw %>%
  rename(sex_desc = sex,
         ...
         special_needs_code = cod_int_alu, # ADHD, blindness, etc. 0 = none. 105 = ADHD, 203 = ADHD. See ER_Matricula_por_alumno_PUBL_MRUN annex 7
...
get_grouped_prev_plot <- function(x, grouping_vars) {
  ...
  # x = chile_bayes_ADHD, needs columns called ADHD, count
  ...
  x_grouped <- x %>% 
    ...
    mutate(n_ADHD = ifelse(is.na(n_ADHD), 0, n_ADHD), # If there are no cases of ADHD in the group, input 0
           sample_pop_size = n_noADHD + n_ADHD, # Total sample population is ADHD cases + not cases
           sample_prevalence = n_ADHD / sample_pop_size, # Prevalence of ADHD in the group
...
get_grouped_prev <- function(x, stdpop, grouping_vars) {
  ...
  x_grouped <- x %>% 
    ...
    mutate(ADHD_prev_std = n_ADHD / sample_pop_size * pop_prop, # Prevalence of ADHD in the group, standardised to standard population
...
do_jags_rand_model <- function(x, feat, model, theta_mu, theta_sigma, pars, nBurn = 1000, nIter = 1000, convergence_checks = FALSE) {
  ...
  if(convergence_checks) {
    ...
    print(mcmc_trace(rand_sam, paste0("ADHD_pred[", 1:nFeat, "]"))) # Convergence looks fine and rhats <= 1.1
...
nIter <- 1000
nBurn <- 1000
pars <- c("theta_a", "theta_b", "theta", "ADHD_sample", "ADHD_pred")

theta_mu_prior <- ADHD_prev_adj$adjusted_rate
theta_sigma_prior <- (ADHD_prev_adj$ci_upper - ADHD_prev_adj$ci_lower) / (2*1.96)
theta_mu_extrapolate <- c(0.02, 0.03, 0.05) # 0.5%, 1%, 1.5%, 3% prevalence
theta_sigma_extrapolate <- c(rep(0.0001/1.96, 3)) # Same as chosen prior
...
# Say ADHD has mean prevalence of 3% and we are 95% confidence that the prevalence is between 2% and 4%.
...
common_model <- "model {
  ...
  #ADHD_post <- ADHD_sample/nObs * sens + (1 - ADHD_sample/nObs) * spec
}"

common_data <- list(theta_a = theta_a_common, 
                    theta_b = theta_b_common,
                    nObs = nObs,
                    #ADHD_sample = sum(chile_bayes_ADHD$ADHD) # unadjusted sample prevalence
                    ADHD_sample = ADHD_prev_adj$adjusted_count # age- and sex- adjusted sample prevalence
                    #spec_mu = 0.996,
...
ADHD_prev_f <- chile_bayes_ADHD %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, grouping_vars = c("age_june30", "sex", "ADHD"))
ADHD_prev_adj_f <- get_adjusted_prev(ADHD_prev_f, grouping_vars = c()) %>% mutate(sex_desc = "Female")

ADHD_prev_m <- chile_bayes_ADHD %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, grouping_vars = c("age_june30", "sex", "ADHD"))
...
plot_post_density(ADHD_prev_sex_post, ADHD_prev_sex_adj, feat = "sex_desc", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
ADHD_prev_region.agecat <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_region_name_abr", "age_cat_name", "ADHD"))

ggplot(data = ADHD_prev_region.agecat) +
  ...
  labs(title = "ADHD prevalence by school region",
       x = "Age category",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_region.age <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_region_name_abr", "age_june30", "ADHD"))

ggplot(data = ADHD_prev_region.age) +
...
ADHD_prev_region.agecat.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_region_name_abr", "age_cat_name", "sex_desc", "ADHD"))

ggplot(data = ADHD_prev_region.agecat.sex) +
  ...
  #theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~school_region_name_abr) +
  labs(title = "ADHD prevalence by sex",
       x = "Sex",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_region.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_region_name_abr", "sex_desc", "ADHD"))

ggplot(data = ADHD_prev_region.sex) +
...
ADHD_prev_region.geom.f <- ADHD_prev_region.sex %>%
...
ggplot(ADHD_prev_region.geom.f) + 
  ...
  labs(title = "ADHD prevalence (females)") +
  xlab("Longitude") +
  ylab("Latitude") +
  coord_sf(xlim = c(-83, -55), ylim = c(-60, -15))

ADHD_prev_region.geom.m <- ADHD_prev_region.sex %>%
  filter(sex_desc == "Male") %>%
  left_join(regions_lookup, by = "school_region_name_abr") %>%
  left_join(demog_geom, by = c("nombre_region", "codigo_region"))

ggplot(ADHD_prev_region.geom.m) + 
  geom_sf(mapping = aes(geometry = geometry, fill = sample_prevalence), linewidth = 0.01) +
  geom_sf_text(mapping = aes(geometry = geometry, label = nombre_region), nudge_x = 8, size = 3) +
  scale_fill_viridis(option = "plasma", name = "Prevalence") + #, limits = c(0, 0.025)) +
  labs(title = "ADHD prevalence (males)") +
  xlab("Longitude") +
  ylab("Latitude") +
  coord_sf(xlim = c(-83, -55), ylim = c(-60, -15))
...
```{r region}
ADHD_prev_region <- get_grouped_prev(x = chile_bayes_ADHD, stdpop = chile_stdpop, 
                                    grouping_vars = c("school_region_name_abr", "age_june30", "age_cat_name", "sex", "sex_desc", "ADHD"))

ADHD_prev_region_adj <- get_adjusted_prev(ADHD_prev_region, grouping_vars = "school_region_name_abr")

ggplot(data = ADHD_prev_region_adj) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Age- and sex-adjusted ADHD prevalence",
       x = "School region",
       y = "Sample prevalence")

ADHD_prev_region_post <- do_jags_rand_model(x = ADHD_prev_region_adj,
...
plot_post_density(ADHD_prev_region_post, ADHD_prev_region_adj, feat = "school_region_name_abr", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_region_post, 
                    ADHD_prev_region_adj, 
                    feat = "school_region_name_abr", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j])
}
...
```{r sex priors}
theta_mu_prior_f <- ADHD_prev_adj_f$adjusted_rate
theta_sigma_prior_f <- sqrt(ADHD_prev_adj_f$var)

theta_mu_prior_m <- ADHD_prev_adj_m$adjusted_rate
theta_sigma_prior_m <- sqrt(ADHD_prev_adj_m$var)

```

```{r region by sex}
# Females
ADHD_prev_region_f <- chile_bayes_ADHD %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, grouping_vars = c("school_region_name_abr", "age_june30", "sex", "ADHD"))

ADHD_prev_region_adj_f <- get_adjusted_prev(ADHD_prev_region_f, grouping_vars = "school_region_name_abr")

ADHD_prev_region_post_f <- do_jags_rand_model(x = ADHD_prev_region_adj_f,
                              feat = "school_region_name_abr",
                              model = rand_model,
...
plot_post_density(ADHD_prev_region_post_f, ADHD_prev_region_adj_f, feat = "school_region_name_abr", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")

# Males
ADHD_prev_region_m <- chile_bayes_ADHD %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, grouping_vars = c("school_region_name_abr", "age_june30", "sex", "ADHD"))

ADHD_prev_region_adj_m <- get_adjusted_prev(ADHD_prev_region_m, grouping_vars = "school_region_name_abr")

ADHD_prev_region_post_m <- do_jags_rand_model(x = ADHD_prev_region_adj_m,
                              feat = "school_region_name_abr",
                              model = rand_model,
...
plot_post_density(ADHD_prev_region_post_m, ADHD_prev_region_adj_m, feat = "school_region_name_abr", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_region_post_f, 
                    ADHD_prev_region_adj_f, 
                    feat = "school_region_name_abr", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (females)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_region_post_m, 
                    ADHD_prev_region_adj_m, 
                    feat = "school_region_name_abr", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (males)")
...
```{r health to region}
map_tab_df <- as.data.frame(table(chile_bayes_ADHD$school_region_name_abr, chile_bayes_ADHD$health_service_name))
...
#map2_tab_df <- as.data.frame(table(chile_bayes_ADHD$school_commune_name, chile_bayes_ADHD$health_service_name))
...
ADHD_prev_health.agecat.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("health_service_name", "age_cat_name", "sex_desc", "ADHD"))
...
ggplot(data = ADHD_prev_health.agecat.sex) +
  ...
  labs(title = "ADHD prevalence by school health service",
       x = "Sex",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_health.age.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("health_service_name", "age_june30", "sex_desc", "ADHD"))
...
ADHD_prev_health <- get_grouped_prev(x = chile_bayes_ADHD, stdpop = chile_stdpop, 
                                    grouping_vars = c("health_service_name", "age_june30", "age_cat_name", "sex", "sex_desc", "ADHD"))

ADHD_prev_health_adj <- get_adjusted_prev(ADHD_prev_health, grouping_vars = "health_service_name")

# ggplot(data = ADHD_prev_health_adj) +
...
plot_post_density(ADHD_prev_health_post, ADHD_prev_health_adj, feat = "health_service_name", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_health_post, 
...
ADHD_prev_rural.agecat.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_rurality_code", "age_cat_name", "sex_desc", "ADHD")) %>%
...
ggplot(data = ADHD_prev_rural.agecat.sex) +
  ...
  labs(title = "ADHD prevalence by school rurality",
       x = "Sex",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_rural.age.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_rurality_code", "age_june30", "sex_desc", "ADHD")) %>%
  mutate(rurality_desc = ifelse(school_rurality_code == 0, "Urban", "Rural"))
...
ADHD_prev_rural <- get_grouped_prev(x = chile_bayes_ADHD, stdpop = chile_stdpop, 
                                    grouping_vars = c("school_rurality_code", "age_june30", "age_cat_name", "sex", "sex_desc", "ADHD")) %>%
  mutate(rurality_desc = ifelse(school_rurality_code == 0, "Urban", "Rural"))

ADHD_prev_rural_adj <- get_adjusted_prev(ADHD_prev_rural, grouping_vars = "rurality_desc")

# ggplot(data = ADHD_prev_rural_adj) +
...
plot_post_density(ADHD_prev_rural_post, ADHD_prev_rural_adj, feat = "rurality_desc", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_rural_post, 
                    ADHD_prev_rural_adj, 
                    feat = "rurality_desc", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j])
...
ADHD_prev_ethnic.agecat.sex <- get_grouped_prev_plot(x = filter(chile_bayes_ADHD, school_region_name_abr %in% 
...
ADHD_prev_ethnic.age.sex <- get_grouped_prev_plot(x = filter(chile_bayes_ADHD, school_region_name_abr %in% 
                                                                 c("ARAUC", "RM", "LAGOS", "BBIO", "RIOS", "MAG", "AYSEN")),
                                                                 #c("ARAUC", "BBIO", "LAGOS", "RIOS", "RM")),
...
ADHD_prev_ethnic <- chile_bayes_ADHD %>%
...
# ggplot(data = ADHD_prev_ethnic_adj) +
...
plot_post_density(ADHD_prev_ethnic_post, ADHD_prev_ethnic_adj, feat = "ethnic_2_group", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_ethnic_post, 
...
```{r ethnicity by sex}
# Females
ADHD_prev_ethnic_f <- chile_bayes_ADHD %>%
  filter(school_region_name_abr %in% c("ARAUC", "RM", "LAGOS", "BBIO", "RIOS", "MAG", "AYSEN")) %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, grouping_vars = c("ethnic_2_group", "age_june30", "sex", "ADHD"))

ADHD_prev_ethnic_adj_f <- get_adjusted_prev(ADHD_prev_ethnic_f, grouping_vars = "ethnic_2_group")

ADHD_prev_ethnic_post_f <- do_jags_rand_model(x = ADHD_prev_ethnic_adj_f,
                              feat = "ethnic_2_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_f,
                              theta_sigma = theta_sigma_prior_f,
...
plot_post_density(ADHD_prev_ethnic_post_f, ADHD_prev_ethnic_adj_f, feat = "ethnic_2_group", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")

# Males
ADHD_prev_ethnic_m <- chile_bayes_ADHD %>%
  filter(school_region_name_abr %in% c("ARAUC", "RM", "LAGOS", "BBIO", "RIOS", "MAG", "AYSEN")) %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, grouping_vars = c("ethnic_2_group", "age_june30", "sex", "ADHD"))

ADHD_prev_ethnic_adj_m <- get_adjusted_prev(ADHD_prev_ethnic_m, grouping_vars = "ethnic_2_group")

ADHD_prev_ethnic_post_m <- do_jags_rand_model(x = ADHD_prev_ethnic_adj_m,
                              feat = "ethnic_2_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_m,
                              theta_sigma = theta_sigma_prior_m,
...
plot_post_density(ADHD_prev_ethnic_post_m, ADHD_prev_ethnic_adj_m, feat = "ethnic_2_group", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_ethnic_post_f, 
                    ADHD_prev_ethnic_adj_f, 
                    feat = "ethnic_2_group", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (females)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_ethnic_post_m, 
                    ADHD_prev_ethnic_adj_m, 
                    feat = "ethnic_2_group", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (males)")
...
ADHD_prev_econA.agecat.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_fee", "age_cat_name", "sex_desc", "ADHD"))
...
ggplot(data = ADHD_prev_econA.agecat.sex) +
  ...
  labs(title = "ADHD prevalence by economic status (all)",
       x = "Sex",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_econA.age.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_fee", "age_june30", "sex_desc", "ADHD"))
...
ADHD_prev_econ <- get_grouped_prev(x = chile_bayes_ADHD, stdpop = chile_stdpop, 
                                  grouping_vars = c("school_fee", "school_fee_group", "age_june30", "age_cat_name", "sex", "sex_desc", "ADHD"))

ADHD_prev_econ_adj <- get_adjusted_prev(ADHD_prev_econ, grouping_vars = "school_fee")

# ggplot(data = ADHD_prev_econ_adj) +
...
ADHD_prev_econ_post <- do_jags_rand_model(x = ADHD_prev_econ_adj,
                              feat = "school_fee",
                              model = rand_model,
                              theta_mu = theta_mu_prior,
                              theta_sigma = theta_sigma_prior,
...
plot_post_density(ADHD_prev_econ_post, ADHD_prev_econ_adj, feat = "school_fee", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_econ_post, 
                    ADHD_prev_econ_adj, 
                    feat = "school_fee", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j])
...
ADHD_prev_econG.agecat.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_fee_group", "age_cat_name", "sex_desc", "ADHD"))

ggplot(data = ADHD_prev_econG.agecat.sex) +
  geom_col(aes(x = sex_desc, y = sample_prevalence, group = age_cat_name, fill = age_cat_name), position = "dodge") +
  geom_errorbar(aes(x = sex_desc, ymin = ci_lower, ymax = ci_upper, group = age_cat_name), width = 0.2, position = position_dodge(width = 0.9)) +
  scale_fill_viridis_d(option = "plasma") +
  #theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.2)) +
  facet_wrap(~school_fee_group) +
  labs(title = "ADHD prevalence by economic status (grouped)",
       x = "Sex",
       y = "Sample prevalence",
       fill = "Age category")

ADHD_prev_econG.age.sex <- get_grouped_prev_plot(x = chile_bayes_ADHD, c("school_fee_group", "age_june30", "sex_desc", "ADHD"))

ggplot(data = ADHD_prev_econG.age.sex) +
  geom_col(aes(x = sex_desc, y = sample_prevalence, group = age_june30, fill = age_june30), position = position_dodge()) +
  geom_errorbar(aes(x = sex_desc, ymin = ci_lower, ymax = ci_upper, group = age_june30), width = 0.2, position = position_dodge(width = 0.9)) +
  scale_fill_viridis_c(option = "plasma") +
  #theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.2)) +
...
ADHD_prev_econG_adj <- get_adjusted_prev(ADHD_prev_econ, grouping_vars = "school_fee_group")

# ggplot(data = ADHD_prev_econG_adj) +
...
ADHD_prev_econG_post <- do_jags_rand_model(x = ADHD_prev_econG_adj,
...
plot_post_density(ADHD_prev_econG_post, ADHD_prev_econG_adj, feat = "school_fee_group", theta_mu = theta_mu_prior, theta_sigma = theta_sigma_prior)
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_econG_post, 
                    ADHD_prev_econG_adj, 
                    feat = "school_fee_group", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j])
...
Use grouped fees because they ungrouped classes are too small and there are no females in the 1,000-10,000 group.

```{r economicGroup by sex}
# Females
ADHD_prev_econG_f <- chile_bayes_ADHD %>%
  filter(sex == 2) %>%
  get_grouped_prev(stdpop = chile_stdpop_f, grouping_vars = c("school_fee_group", "age_june30", "sex", "ADHD"))

ADHD_prev_econG_adj_f <- get_adjusted_prev(ADHD_prev_econG_f, grouping_vars = "school_fee_group")

ADHD_prev_econG_post_f <- do_jags_rand_model(x = ADHD_prev_econG_adj_f,
                              feat = "school_fee_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_f,
                              theta_sigma = theta_sigma_prior_f,
...
plot_post_density(ADHD_prev_econG_post_f, ADHD_prev_econG_adj_f, feat = "school_fee_group", theta_mu = theta_mu_prior_f, theta_sigma = theta_sigma_prior_f, title_text = " (females)")

# Males
ADHD_prev_econG_m <- chile_bayes_ADHD %>%
  filter(sex == 1) %>%
  get_grouped_prev(stdpop = chile_stdpop_m, grouping_vars = c("school_fee_group", "age_june30", "sex", "ADHD"))

ADHD_prev_econG_adj_m <- get_adjusted_prev(ADHD_prev_econG_m, grouping_vars = "school_fee_group")

ADHD_prev_econG_post_m <- do_jags_rand_model(x = ADHD_prev_econG_adj_m,
                              feat = "school_fee_group",
                              model = rand_model,
                              theta_mu = theta_mu_prior_m,
                              theta_sigma = theta_sigma_prior_m,
...
plot_post_density(ADHD_prev_econG_post_m, ADHD_prev_econG_adj_m, feat = "school_fee_group", theta_mu = theta_mu_prior_m, theta_sigma = theta_sigma_prior_m, title_text = " (males)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_econG_post_f, 
                    ADHD_prev_econG_adj_f, 
                    feat = "school_fee_group", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (females)")
...
for(j in 1:length(theta_mu_extrapolate)) {
  ...
  plot_post_density(ADHD_prev_econG_post_m, 
                    ADHD_prev_econG_adj_m, 
                    feat = "school_fee_group", 
                    theta_mu = theta_mu_extrapolate[j],
                    theta_sigma = theta_sigma_extrapolate[j],
                    title_text = " (males)")
...
## Rerun features that are most interesting using prevalence from clinical data, with sexes separate. Then graph prevalences and see if CI's overlap.
