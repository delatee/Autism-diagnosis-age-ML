...
patients_mca <- clinical %>% 
  ...
  mutate(medical_specialty_english = ifelse(medical_specialty == "Physiatry", "Psychiatry",
         ...
         medical_specialty_grouped = ifelse(medical_specialty_english %in% c("Psychiatry", "Child Psychiatry"), "Psychiatry",
                                     ...
                                     ifelse(medical_specialty_english == "Paediatrics", "Paediatrics", "Other"))),
  ...
  mutate(sex_desc = as.factor(sex_desc),
         age_group = as.factor(ifelse(age <= 2, "0-2", 
                               ifelse(age >= 3 & age <= 5, "3-5",
                               ifelse(age >=6 & age <= 8, "6-8", 
                               ifelse(age >=9 & age <=11, "9-11", 
                               ifelse(age >=12 & age <= 14, "12-14", 
                               ifelse(age >= 15 & age <= 18, "15-18", "Adult"))))))),
         age = as.factor(age),
         commune_name = as.factor(commune_name),
...
res_mca_patients <- FactoMineR::MCA(patients_mca, # Can only take categorical variables and they have to be cast to factors
                                    ncp = 5, # Needs to match number of features
                                    graph = TRUE) 
