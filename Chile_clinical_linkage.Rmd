...
school_matched_small <- school_matched %>%
  ...
  select(id.school, dob.school, sex_desc.school, sex.school, student_commune_name.school, ses_status.school, matched)
...
# Explore distributions across matched/non-matched
# Sex
data1 <- school_matched_small %>% filter(matched == 1) #%>% select(sex.school)
data2 <- school_matched_small %>% filter(matched == 0) #%>% select(sex.school)
#hist(data1$sex.school)
#hist(data2$sex.school)
data1 %>% group_by(sex.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count))
data2 %>% group_by(sex.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count))
ks.sex <- ks.test(data1$sex.school, data2$sex.school, alternative = "two.sided", simulate.p.value = TRUE)
ks.sex
...
set.seed(123)
nPerm <- 2000
ks_pvals <- data.frame(sex = numeric(nPerm),
                       ses = numeric(nPerm))
...
for (i in 1:nPerm) {
  ...
  ks.sex <- ks.test(datayes$sex.school, datano$sex.school, alternative = "two.sided")
  ks.ses <- ks.test(datayes$ses_status.school, datano$ses_status.school, alternative = "two.sided")
  ks_pvals$sex[i] <- ks.sex$p.value
  ks_pvals$ses[i] <- ks.ses$p.value
}

plot(density(ks_pvals$sex))
#hist(ks_pvals$sex, breaks = 30)
plot(density(ks_pvals$ses))
#hist(ks_pvals$ses, breaks = 30)


```

Our matched/non-matched are not different by sex (p-value in Kolmog is same as most of distribution of permuted pvals) but are different by ses status


Probably not suitable to use K-S because data are not continuous. Try Chi2 instead. Or simulated K-S to choose our own p-value threshold.


Now analyse the matched and unmatched school records and see if they are fundamentally different. ?Cohen test?
