...
clinical_small <- clinical_small_raw %>%
  ...
  mutate(gender = str_to_title(gender),
         autism = ifelse(codigo %in% aut_codes | 
                           cod_dg_2 %in% aut_codes | 
                           cod_dg_3 %in% aut_codes, 1, 0),
         intdisab = ifelse(codigo %in% intdisab_codes |
                             cod_dg_2 %in% intdisab_codes | 
                             cod_dg_3 %in% intdisab_codes, 1, 0),
         aut_rank = ifelse(codigo %in% aut_codes, 1,
...
merged %>% filter(!is.na(id.x) & !is.na(id.y)) # 85 matches
