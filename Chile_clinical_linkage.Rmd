...
patients <- clinical %>%
  ...
  rename("student_commune_name" = "commune_name",
         "sex_desc" = "gender") %>%
  select(id, dob, sex_desc, student_commune_name, ses_status) #, count)
...
a_pairs <- a$pairs
b <- emWeights(a, cutoff = c(0.99, 0.99, 0.85, 0.6))
summary(b)
allPairs <- getPairs(b)
head(allPairs, n = 20)
...
a2_pairs <- a2$pairs
b2 <- epiWeights(a2, e = c(0.01, 0.01, 0.01, 0.3)) # Have more error for ses_status. 0.01 is default
summary(b2)
