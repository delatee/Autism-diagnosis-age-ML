...
https://www.tibco.com/reference-center/what-is-correspondence-analysis#:~:text=Correspondence%20analysis%2C%20also%20called%20reciprocal,two%20or%20more%20data%20points.
https://ladal.edu.au/clust.html
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3810057/
http://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/

```{r MCA clinical}

clinical_mca <- clinical %>%
  mutate(gender = as.factor(gender),
         age_june30 = as.factor(age_june30),
         commune_name = as.factor(commune_name),
         appt_type = as.factor(type_appointment),
         ses_status = as.factor(ifelse(socio_economic_level %in% c("COLMENA GOLDEN CROSS", "RIO BLANCO", "CARABINEROS (DIPRECA)", "BANMEDICA S.A.", "PARTICULAR (SIN PREVISION)", "VIDA TRES"), "Private Health Insurance", socio_economic_level)),
...
# Test if there is sufficient structure in the data to do clustering
#clusttendency <- get_clust_tendency(clinical_mca,
#                                    n = 5, # Needs to be same as number of features
#                                    gradient = list(low = "steelblue", high = "white"))  
# Can only handle numeric data so not that useful but even with only the numeric features,
# the Hopkins stat is very close to 1 (0.997) so the data is highly clusterable

res_mca_clinical <- FactoMineR::MCA(clinical_mca, ncp = 4, graph = TRUE) # Can only take categorical variables and they have to be cast to factors
fviz_mca_var(res_mca_clinical, choice = "mca.cor", 
            repel = TRUE, # Avoid text overlapping (slow)
            ggtheme = theme_minimal())

```

Probably better to do MCA on patient-level data because there will be a substructure of clusters of the patients' appointments.
Especially as there are some duplicates in the clinical data, seemingly as a result of translating the medical_specialty vales
...
patients_mca <- clinical %>% 
  #filter(commune_code %in% araucsur_communes$commune_code) %>%
  mutate(medical_specialty_english = ifelse(medical_specialty == "Physiatry", "Psychiatry",
  ...
  group_by(id, gender, dob, commune_name) %>%
  summarise(ses_status = get.min.na(ses_status), # Fortunately the available values are in alphabetical order by status
            ...
            age = age_june30[which.max(apt_date)] # Age should be the same for all appts because it's based on dob
            ) %>%
  ungroup() %>% 
...
res_mca_patients <- FactoMineR::MCA(patients_mca, # Can only take categorical variables and they have to be cast to factors
                                    ncp = 7, # Needs to match number of features
                                    graph = TRUE) 

# Plot features
fviz_mca_var(res_mca_patients, choice = "mca.cor", 
            repel = TRUE, # Avoid text overlapping
            ggtheme = theme_minimal())

# Plot categories
fviz_mca_var(res_mca_patients,
             col.var = "cos2",
...
fviz_mca_ind(res_mca_patients, col.ind = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = FALSE, # Graphs is not meaningful if this is TRUE
             ggtheme = theme_minimal())
# OR
fviz_mca_ind(res_mca_patients, 
...
fviz_mca_ind(res_mca_patients, 
             label = "none", # hide individual labels
             habillage = "hospital", # color by groups 
             #palette = c("#00AFBB", "#E7B800"),
             addEllipses = TRUE, ellipse.type = "confidence",
...
fviz_mca_ind(res_mca_patients, 
             label = "none", # hide individual labels
             habillage = "medical_specialty_grouped", # color by groups 
             #palette = c("#00AFBB", "#E7B800"),
             addEllipses = TRUE, ellipse.type = "confidence",
             title = "Patients by most common medical specialty",
             ggtheme = theme_minimal())
...
fviz_cos2(res_mca_patients, choice = "var", axes = 1:2)
round(res_mca_patients$var$contrib,2) # "The variable categories with the larger value, contribute the most to the definition of the dimensions. Variable categories that contribute the most to Dim.1 and Dim.2 are the most important in explaining the variability in the data set."

# Contributions of variable categories to dimension 1
fviz_contrib(res_mca_patients, choice = "var", axes = 1, top = 15) 
# Contributions of variable categories to dimension 2
fviz_contrib(res_mca_patients, choice = "var", axes = 2, top = 15)
# Total contribution to dimension 1 and 2
fviz_contrib(res_mca_patients, choice = "var", axes = 1:2, top = 15)
...
res_mca_patients_small <- FactoMineR::MCA(patients_mca_small, # Can only take categorical variables and they have to be cast to factors
                                    ncp = 8, # Needs to match number of features
                                    graph = TRUE) 

# Plot features
fviz_mca_var(res_mca_patients_small, choice = "mca.cor", 
            repel = TRUE, # Avoid text overlapping
            ggtheme = theme_minimal())

# Plot categories
fviz_mca_var(res_mca_patients_small,
             col.var = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, # Avoid text overlapping (slow if many points)
             ggtheme = theme_minimal())
...
# Plot individual patients
fviz_mca_ind(res_mca_patients_small, col.ind = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = FALSE, # Graphs is not meaningful if this is TRUE
             ggtheme = theme_minimal())
# OR
fviz_mca_ind(res_mca_patients_small, 
             label = "none", # hide individual labels
             habillage = "sex_desc", # color by groups 
             palette = c("green", "blue"),
             addEllipses = TRUE, ellipse.type = "confidence",
...
fviz_cos2(res_mca_patients_small, choice = "var", axes = 1:2)
...
# Contributions of variable categories to dimension 1
fviz_contrib(res_mca_patients_small, choice = "var", axes = 1, top = 15) 
# Contributions of variable categories to dimension 2
fviz_contrib(res_mca_patients_small, choice = "var", axes = 2, top = 15)
# Total contribution to dimension 1 and 2
fviz_contrib(res_mca_patients_small, choice = "var", axes = 1:2, top = 15)
# "The red dashed line on the graph above indicates the expected average value, If the contributions were uniform" See http://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/ for calculations
...
#   mutate(sex = ifelse(gender == "Male", 1, ifelse(gender == "Female", 2, NA)),
...
#   select(id, sex, age_june30, ses_detailed, appt_type)
