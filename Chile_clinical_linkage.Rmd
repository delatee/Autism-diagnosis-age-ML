                                   ...
                                   ifelse(socio_economic_level == "FONASA - C", 2, 
                                          ifelse(socio_economic_level == "FONASA - D", 2,
                                                 ifelse(socio_economic_level == "Private Health Insurance", 3, 
                                                        ifelse(socio_economic_level %in% c("COLMENA GOLDEN CROSS", "RIO BLANCO", "CARABINEROS (DIPRECA)", "BANMEDICA S.A.", "PARTICULAR (SIN PREVISION)", "VIDA TRES"), 0, NA)))))),
         autism = 1,
         intdisab = 0
         ) %>%
  left_join(commune_region_lookup, by = "commune_name") %>%
  select(id, gender, commune_name, health_service_name, region_name, socio_economic_level, ses_status, dob, age_june30, apt_date, hospital, medical_specialty, type_appointment, codigo, diagnosis, autism, intdisab) 

clinical_small_raw <- read_excel("04_Data/Dataset_Vill_2014_2021.xlsx", col_names = TRUE) %>% clean_names()

sort(unique(clinical_large$codigo))
...
clinical_small <- clinical_small_raw %>%
  ...
  mutate(gender = str_to_title(gender),
         autism = ifelse(codigo %in% unique(clinical_large$codigo) | 
                           cod_dg_2 %in% unique(clinical_large$codigo) | 
                           cod_dg_3 %in% unique(clinical_large$codigo), 1, 0),
...
patients <- clinical %>%
  group_by(id, gender, commune_name, region_name, dob, ses_status, autism, intdisab) %>%
  summarise(count = n()) %>%  # Might be better to take min(ses_status) here
  ungroup() %>% 
  rename("student_commune_name" = "commune_name",
         "student_region_name" = "region_name",
         "sex_desc" = "gender") %>%
  rowid_to_column("row_id") %>%
  select(row_id, id, dob, sex_desc, student_commune_name, ses_status, autism, intdisab) #, student_region_name) #, count)
...
a2 <- compare.linkage(school,
                     ...
                     select(patients, -row_id),
                     #select(patients, -ses_status), 
                     blockfld = c("sex_desc", "dob"), # Block on sex because we really want it to be the same and help computation time
                     #blockfld = FALSE,
                     phonetic = FALSE,
                     strcmp = c(2), # Do string comparison on DOB
                     exclude = c(1) # Exclude the id column in both datasets
...
a2_pairs <- a2$pairs
b2 <- epiWeights(a2, e = c(0.01, 0.01, 0.01, 0.3)) #, 0.01)) # Have more error for ses_status. 0.01 is default
summary(b2)
...
pairs <- pair_blocking(school, patients, on = c("sex_desc", "dob")) %>%
  ...
  select(c(-student_commune_name.x, -ses)) %>%
