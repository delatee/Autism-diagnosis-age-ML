...
```{r commune lookup}
commune_region_lookup <- chile_merged %>%
  group_by(school_region_name_abr, school_commune_name) %>%
  summarise() #%>%
  #rename("region" = "school_region_name_abr", "commune" = "school_commune_name")
...
clinical <- clinical_raw %>%
  ...
  mutate(dob_eng = ifelse(str_detect(date_of_birth, "/"), 1, ifelse(str_detect(date_of_birth, "-"), 0, NA)),
         ...
         school_commune_name = ifelse(comuna == "CHOL CHOL", "CHOLCHOL",
                            ...
                            ifelse(socio_economic_level == "FONASA - B", 2,
                                   ifelse(socio_economic_level == "FONASA - C", 3, 
                                          ifelse(socio_economic_level == "FONASA - D", 4,
                                                 ifelse(socio_economic_level == "Private Health Insurance", 5, NA)))))
         ) %>%
  left_join(commune_region_lookup, by = "school_commune_name") %>%
  select(id, gender, school_commune_name, school_region_name_abr, socio_economic_level, ses_status, dob, age_june30, apt_date, hospital, medical_specialty, type_appointment, codigo, diagnosis) #%>%
...
clinical_communes <- clinical_small %>% group_by(school_commune_name) %>% summarise() %>% arrange() %>%
  mutate(commune_in_school_data = ifelse(school_commune_name %in% unique(chile_merged$school_commune_name), 1, 0)) # Needs to all be 1's
...
patients <- clinical %>%
  group_by(id, gender, school_commune_name, dob, socio_economic_level, ses_status, age_june30) %>%
  ...
  #       "sex_desc" = "gender") %>%
  left_join(commune_region_lookup, by = "school_commune_name") %>%
  rename("student_commune_name" = "school_commune_name",
         "sex_desc" = "gender") #%>%
...
patients_small <- clinical_small %>%
  ...
  rename("student_commune_name" = "school_commune_name") %>%
  select(c(id, sex_desc, student_commune_name, age_june30, dob))
...
patients_grouped <- patients %>%
  group_by(sex_desc, dob, student_commune_name) %>%
  summarise(count = n())
         ...
         ses_status = ifelse(school_fee == "", 0, 
                             ifelse(school_fee == "GRATUITO", 1, 
                                    ifelse(school_fee == "$1.000 A $10.000", 2,
                                           ifelse(school_fee == "$10.001 A $25.000", 3, 
                                                  ifelse(school_fee == "$25.001 A $50.000", 4,
                                                         ifelse(school_fee == "$50.001 A $100.000", 5, 
                                                                ifelse(school_fee == "MAS DE $100.000", 6, 
...
get_num_diff <- function(.x, .y) {
  ...
  diff_value <- diff_value %>%
    mutate(val = ifelse(abs(.x-.y) == 0, 1, 
                        ifelse(abs(.x-.y) > 0 & abs(.x-.y) <= 1, 0.5, 0)))
...
pairs <- #pair_blocking(patients_small[1:50, ], school[1:50, ], on = "sex_desc") %>% # Also try without the blocking 
         #pair_blocking(patients, school, on = "sex_desc") %>%
...
pairs_match <- pairs %>% filter(dob == 1 & student_commune_name == TRUE & sex_desc == TRUE)
...
pairs_grouped <- pairs %>% group_by(k_pred, sex_desc, student_commune_name, dob, ses_pat, ses_sch) %>% summarise()
# Not very useful
...
set.seed(123)
c <- 3
pairs_trues <- pairs %>% filter(dob == TRUE | student_commune_name == TRUE | sex_desc == TRUE) %>% select(-k_pred) # Remove clearly unmatched rows to help computation time
...
pairs_trues$c_pred <- re_class(cmeans_result$cluster)
