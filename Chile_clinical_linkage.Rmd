...
clinical <- clinical_raw %>%
  ...
  select(id, gender, school_commune_name, socio_economic_level, dob, age_june30, apt_date, hospital, medical_specialty, type_appointment, codigo, diagnosis) #%>%
...
patients <- clinical %>%
  group_by(id, gender, school_commune_name, dob, age_june30) %>%
  summarise() %>%
  #summarise(first_apt = min(apt_date)) %>%
  ungroup() %>% 
  rename("dob_full" = "dob",
         "sex_desc" = "gender") %>%
  left_join(commune_region, by = "school_commune_name") %>%
...
table(sort(unique(patients_small$school_commune_name, sort(unique(school$school_commune_name)))))
sort(unique(patients_small$school_commune_name))
sort(unique(school$school_commune_name))
...
set.seed(123)
k <- 3 # We know there are 2 classes: matched records and un-matched records, and we'd like a 3rd class for maybes
kmeans_result <- kmeans(select(pairs, c(-.x, -.y)), k, nstart = 20)
pairs$pred <- predict(kmeans_result, select(pairs, c(-.x, -.y)))
table(pairs$pred)
