...
clinical_large <- clinical_large_raw %>%
  ...
  select(id, gender, commune_name, health_service_name, region_name, socio_economic_level, ses_status, dob, age_june30, apt_date, hospital, medical_specialty, type_appointment, codigo, diagnosis, autism, intdisab, aut_rank) 
...
clinical_small <- clinical_small_raw %>%
  ...
  left_join(commune_region_lookup, by = "commune_name") %>%
  #filter(autism == 1) %>%
  select(id, gender, commune_name, health_service_name, region_name, socio_economic_level, ses_status, dob, age_june30, apt_date, hospital, medical_specialty, type_appointment, cod_dg_1, cod_dg_2, cod_dg_3, diagnosis, autism, aut_rank) 
...
b2 <- epiWeights(a2, e = c(0.01, # Default for DOB
                           ...
                           0.1 # Allow some mismatch on whether autism is the primary diagnosis so we take record with higher placed autism diagnosis when there are multiple clinical record options for a single school record
))
summary(b2)
...
# In the case of tied fully-perfect matches, might be better to choose the clinical record with autism as a more primary diagnosis.
...
school_matched_small <- school_matched %>%
  ...
  select(id.school, dob.school, sex_desc.school, sex.school, student_commune_name.school, commune_num, ses_status.school, matched)
...
```{r kolmogorov tests, warning = FALSE}

schoolyes <- school_matched_small %>% filter(matched == 1) #%>% select(sex.school)
schoolno <- school_matched_small %>% filter(matched == 0)

# Kolmogorov tests for our matched results
ks.sex <- ks.test(na.omit(schoolyes$sex.school), na.omit(schoolno$sex.school), alternative = "two.sided", simulate.p.value = TRUE)
ks.sex

ks.ses_status <- ks.test(na.omit(schoolyes$ses_status.school), na.omit(schoolno$ses_status.school), alternative = "two.sided", simulate.p.value = TRUE)
ks.ses_status

ks.commune_num <- ks.test(na.omit(schoolyes$commune_num), na.omit(schoolno$commune_num), alternative = "two.sided", simulate.p.value = TRUE)
ks.commune_num
...
ks_perm_pvals <- data.frame(sex = numeric(nPerm), ses_status = numeric(nPerm))
...
for (i in 1:nPerm) {
  ...
  datayes <- school_matched_small_perm %>% filter(matched == 1)
  datano <- school_matched_small_perm %>% filter(matched == 0)
  
  ks_perm.sex <- ks.test(na.omit(datayes$sex.school), na.omit(datano$sex.school), alternative = "two.sided")
  ks_perm.commune_num <- ks.test(na.omit(datayes$commune_num), na.omit(datano$commune_num), alternative = "two.sided")
  ks_perm.ses_status <- ks.test(na.omit(datayes$ses_status.school), na.omit(datano$ses_status.school), alternative = "two.sided")
  
  ks_perm_pvals$sex[i] <- ks_perm.sex$p.value
  ks_perm_pvals$commune_num[i] <- ks_perm.commune_num$p.value
  ks_perm_pvals$ses_status[i] <- ks_perm.ses_status$p.value
}

# Results for sex
match_yes.sex <- schoolyes %>% group_by(sex.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count), matched = 1)
match_no.sex <- schoolno %>% group_by(sex.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count), matched = 0)
match.sex <- rbind(match_yes.sex, match_no.sex) %>%
...
ggplot(ks_perm_pvals, aes(x = sex, y = after_stat(density))) +
  geom_density() +
  geom_vline(xintercept = ks.sex$p.value, color = "red")

# Results for commune
match_yes.commune_num <- schoolyes %>% group_by(commune_num) %>% summarise(count = n()) %>% mutate(freq = count/sum(count)) %>%
  merge(commune_nums, by = "commune_num", all = TRUE) %>% mutate(matched = 1) 
match_no.commune_num <- schoolno %>% group_by(commune_num) %>% summarise(count = n()) %>% mutate(freq = count/sum(count)) %>%
  merge(commune_nums, by = "commune_num", all = TRUE) %>% mutate(matched = 0)

match.commune_num <- rbind(match_yes.commune_num, match_no.commune_num) %>%
  arrange(commune_num, matched)

ggplot(match.commune_num) +
  geom_col(aes(x = as.factor(matched), y = freq, fill = as.factor(matched))) +
...
ggplot(ks_perm_pvals, aes(x = commune_num, y = after_stat(density))) +
  geom_density() +
  geom_vline(xintercept = ks.commune_num$p.value, color = "red")

# Results for ses status
match_yes.ses_status <- schoolyes %>% group_by(ses_status.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count), matched = 1)
match_no.ses_status <- schoolno %>% group_by(ses_status.school) %>% summarise(count = n()) %>% mutate(freq = count/sum(count), matched = 0)
match.ses_status <- rbind(match_yes.ses_status, match_no.ses_status) %>%
  arrange(ses_status.school, matched)

ggplot(match.ses_status) +
  geom_col(aes(x = as.factor(matched), y = freq, fill = as.factor(matched))) +
...
ggplot(ks_perm_pvals, aes(x = ses_status, y = after_stat(density))) +
  geom_density() +
  geom_vline(xintercept = ks.ses_status$p.value, color = "red")
...
??Add commune maps here with size of sample for school and clinical??
